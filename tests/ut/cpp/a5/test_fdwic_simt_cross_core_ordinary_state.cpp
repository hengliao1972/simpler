...
TEST(FdwicSimtCrossCoreOrdinaryState, AppendsLifecycleAndRequestsAfterTheCommonRuntime) {
    static_assert(PTO_FDWIC_SHARED_MAP == 1);
    static_assert(PTO_FDWIC_SCHEDULER_MODE == 3);
    EXPECT_EQ(kFdwicCrossCoreTaskCapacity, 2048U);
    EXPECT_EQ(offsetof(SimtCrossCoreOrdinaryState, runtime), 0U);
    EXPECT_EQ(offsetof(SimtCrossCoreOrdinaryState, lifecycle), sizeof(CrossCoreRuntimeState));
    EXPECT_EQ(
        offsetof(SimtCrossCoreOrdinaryState, requests),
...
TEST(FdwicSimtCrossCoreOrdinaryState, ResetTouchesControlsButPreservesImmutablePayloadStorage) {
    auto state = std::make_unique<SimtCrossCoreOrdinaryState>();
    std::memset(state.get(), kPayloadPattern, sizeof(*state));

    dist_simt_cross_core_ordinary_reset(*state);
    ...
    EXPECT_TRUE(BytesEqual(
        &state->requests[kFdwicCrossCoreTaskCapacity - 1U], offsetof(fdwic::cross_core::SimtBuildRequestCell, payload),
        sizeof(fdwic::cross_core::SimtBuildRequestStorage), kPayloadPattern
    ));
}
