...
__attribute__((visibility("default"), weak)) PTO_DEVICE_FUNC void aicpu_orchestration_entry(const L2TaskArgs &orch_args) {
    ...
    for (int g = 0; g < num_groups; g++) {
        ...
        Tensor A_view = Tensor::view(ext_A, group_shapes, off);
        Tensor B_view = Tensor::view(ext_B, group_shapes, off);
        Tensor Cmm_view = Tensor::view(ext_Cmm, group_shapes, off);
        Tensor Vfinal_view = Tensor::view(ext_Vfinal, group_shapes, off);
