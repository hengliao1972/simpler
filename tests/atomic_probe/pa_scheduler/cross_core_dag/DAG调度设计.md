...
口径。实现从现有 Scalar [`cross_core_ordinary`](../cross_core_ordinary/) 的成熟代码改造，
但复制到本目录后独立演进，不产生对 `cross_core_ordinary/` 或
[`simt_cross_core_dag`](../simt_cross_core_dag/) 的源码依赖。
...
- 成熟 `cross_core_ordinary`：历史 12 组中位数 `815.937 us`；
