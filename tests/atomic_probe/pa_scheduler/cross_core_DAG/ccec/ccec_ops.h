...
struct CcecOps {
    ...
    __aicore__ static inline bool ExecuteBoundKernel(
        ...
        __gm__ pa_scheduler::cross_core_dag::ExecutionToken &token,
        pa_scheduler::TaskKind kind, uint32_t nop_count
    ) {
        ...
        if (token.dispatch.args[
                pa_scheduler::cross_core_dag::kExecDispatchLocalContextIndex
            ] == 0 ||
            token.dispatch.args[
                pa_scheduler::cross_core_dag::kExecDispatchGlobalContextIndex
            ] == 0) {
            return false;
        }
