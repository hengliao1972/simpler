...
CONTROLLED_CALL = re.compile(r"\b(?:Ops|[A-Za-z_]\w*Ops)\s*::\s*(" + "|".join(CONTROLLED_OPS) + r")\s*\(")
CCEC_INTRINSIC_CALL = re.compile(r"\b(atomicAdd|atomicMax|atomicCAS|dcci)\s*\(")
...
def _strip_cpp_comments_and_literals(source: str) -> str:  # noqa: PLR0912
...
def _is_shared_heap_fallback(call: SourceCall) -> bool:
    ...
    return call.path.name == "pa_shared_heap.h" and expected is not None and call.text == expected


def _parse_atomic_site_enum() -> tuple[dict[str, int], int]:
    source = _strip_cpp_comments_and_literals(MODEL.read_text(encoding="utf-8"))
    match = re.search(
    ...
    for item in match.group(1).split(","):
        ...
        if entry is None:
            raise AssertionError(f"{MODEL}: 无法解析 AtomicSite 条目: {item!r}")
...
def _parse_python_literal_assignments(path: Path, requested: Iterable[str]) -> dict[str, Any]:
    ...
    if missing:
        raise AssertionError(f"{path}: 缺少 Python 映射: {', '.join(sorted(missing))}")
    return values


def _parse_cpp_atomic_site_ops(sites: Mapping[str, int]) -> dict[int, int]:
    source = _strip_cpp_comments_and_literals(MODEL.read_text(encoding="utf-8"))
    function = re.search(
    ...
    for group in groups:
        ...
        if operation not in ATOMIC_OP_IDS:
            raise AssertionError(f"{MODEL}: 未知 AtomicOp::{operation}")
        for name in re.findall(
            ...
            if name not in sites:
                raise AssertionError(f"{MODEL}: AtomicSiteExpectedOp 引用了未知站点 {name}")
...
def _parse_dcci_site_enum() -> tuple[dict[str, int], int]:
    ...
    for item in match.group(1).split(","):
        ...
        entry = re.fullmatch(r"([A-Za-z_]\w*)\s*=\s*(\d+)", item)
        if entry is None:
            raise AssertionError(f"{MODEL}: 无法解析 DcciSite 条目: {item!r}")
        name, raw_value = entry.groups()
        value = int(raw_value)
        if name == "Count":
            count = value
...
def _parse_cpp_dcci_site_ops(sites: Mapping[str, int]) -> dict[int, int]:
    source = _strip_cpp_comments_and_literals(MODEL.read_text(encoding="utf-8"))
    function = re.search(
    ...
    result = {value: DCCI_OP_IDS["Invalidate"] for value in sites.values()}
    groups = re.finditer(
    ...
    for group in groups:
        ...
        for name in re.findall(
            ...
            if name not in sites:
                raise AssertionError(f"{MODEL}: DcciSiteExpectedOp 引用了未知站点 {name}")
...
class AtomicDcciSourceCoverageTest(unittest.TestCase):
    ...
    def test_production_headers_have_no_bare_controlled_ops(self) -> None:
        ...
        if violations:
            ...
            self.fail(f"发现未接入统一观察封装的裸 atomic/DCCI 调用 ({len(violations)}处)：\n{rendered}")
        ...
        for path in sorted(COMMON.glob("*.h")):
            for line_number, line in enumerate(path.read_text(encoding="utf-8").splitlines(), start=1):
                if EXEMPT_PREFIX in line and EXEMPT_MARKER.search(line) is None:
                    malformed.append(f"{path.relative_to(ROOT)}:{line_number}: {line.strip()}")
        if malformed:
            self.fail("发现格式错误或缺少具体原因的源码豁免标记：\n  " + "\n  ".join(malformed))
    ...
    def test_ccec_intrinsics_are_centralized_in_ops(self) -> None:
        ...
        central_calls = 0
        for path in sorted(
            item for item in ccec_dir.rglob("*") if item.suffix in (".h", ".cpp") and "build" not in item.parts
        ):
            searchable = _strip_cpp_comments_and_literals(path.read_text(encoding="utf-8"))
            for match in CCEC_INTRINSIC_CALL.finditer(searchable):
                line = searchable.count("\n", 0, match.start()) + 1
                if path.name == "ccec_ops.h":
                    central_calls += 1
                    continue
                violations.append(f"{path.relative_to(ROOT)}:{line}: {match.group(1)}")
        self.assertGreater(
            central_calls,
            0,
        ...
        if violations:
            self.fail("发现绕过 CcecOps 的设备 atomic/DCCI ISA 调用：\n  " + "\n  ".join(violations))
    ...
    def test_atomic_site_mapping_counts_and_ops_match(self) -> None:
        ...
        python_values = _parse_python_literal_assignments(CONVERTER, ("ATOMIC_SITE_NAMES", "ATOMIC_SITE_OP_IDS"))
    ...
    def test_dcci_site_mapping_counts_and_ops_match(self) -> None:
        ...
        python_values = _parse_python_literal_assignments(CONVERTER, ("DCCI_SITE_NAMES", "DCCI_SITE_OP_IDS"))
