...
# 目录本身就是 Scalar cross-core DAG shared PA 的构建身份，不再接收 private/shared
...
echo "[CHECK] Scalar cross-core DAG atomic/DCCI source coverage"
PA_ATOMIC_DCCI_COVERAGE_ROOT="$ROOT_DIR" \
    "${PYTHON:-python3}" \
    "$ROOT_DIR/test_atomic_dcci_source_coverage.py"
...
# 因此这些门槛仍不能代替后面的完整 96-worker Submit 测试。
    ...
    echo "[BUILD] PA Scalar cross-core DAG execution adapter self-test"
    "$CXX_BIN" -O2 -std=c++17 -pthread -Wall -Wextra -Werror \
        -DPTO_FDWIC_SHARED_MAP=1 \
        -DPA_BUILD_SWIMLANE=1 \
        -I"$ROOT_DIR/common" \
    ...
    echo "[TEST] PA Scalar cross-core DAG execution adapter self-test"
    ...
    echo "[BUILD] PA Scalar cross-core DAG execution scan/drain self-test"
    "$CXX_BIN" -O2 -std=c++17 -pthread -Wall -Wextra -Werror \
        -DPTO_FDWIC_SHARED_MAP=1 \
        -DPA_BUILD_PERF_CLOCK=1 \
        -I"$ROOT_DIR/common" \
        "$ROOT_DIR/test/test_cross_core_dag_exec_scan.cpp" \
        -o "$BUILD_DIR/test_cross_core_dag_exec_scan"

    echo "[TEST] PA Scalar cross-core DAG execution scan/drain self-test"
    "$BUILD_DIR/test_cross_core_dag_exec_scan"
