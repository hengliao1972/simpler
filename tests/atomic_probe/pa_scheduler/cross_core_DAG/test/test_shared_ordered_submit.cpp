...
struct OrderedSubmitTestOps {
    ...
    static bool ExecuteBoundKernel(
        ...
        cross_core_dag::ExecutionToken &token,
        TaskKind kind, uint32_t
    ) {
        const cross_core_dag::ExecPayloadHeader header =
            cross_core_dag::ExecutionTokenHeader(token);
        ...
        const bool valid =
            ...
            token.dispatch.args[
                cross_core_dag::kExecDispatchLocalContextIndex
            ] != 0 &&
            token.dispatch.args[
                cross_core_dag::kExecDispatchGlobalContextIndex
            ] != 0;
...
bool RunInsertReleaseBeforeBuildTest() {
    ...
    const cross_core_dag::DecodedExecFatal exec_fatal =
...
bool RunRemoteFatalCadenceClosureTest() {
    ...
    const bool ok =
        ...
        state->fatal.value == 1 &&
        exec_fatal.reason == cross_core_dag::ExecFatalReason::None &&
        all_workers_returned &&
        total_attempts <=
