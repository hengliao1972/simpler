    ...
    for (uint32_t group = 0;
        ...
        state->exec_drain.arrivals[group].state =
            static_cast<int64_t>(
                6U +
                (completions <<
                 cross_core_dag::kExecDrainArrivalCountBits)
            );
    }
...
int main() {
    ...
    if (g_failures != 0) {
        std::fprintf(
            stderr, "[FAIL] Scalar cross-core DAG Execute ticket: %d checks failed\n",
            g_failures
        );
        return 1;
    }
    std::printf("[PASS] Scalar cross-core DAG Execute ticket and drain closure\n");
    return 0;
}
