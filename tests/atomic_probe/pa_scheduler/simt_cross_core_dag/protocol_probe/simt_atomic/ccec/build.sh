...
DEVICE_OBJECT="$BUILD_DIR/simt_cross_core_atomic_aiv.o"
DEVICE_BITCODE="$BUILD_DIR/simt_cross_core_atomic_aiv.bc"
BITCODE_DUMP="$BUILD_DIR/simt_cross_core_atomic_aiv.bc.dump"
KERNEL_ELF="$BUILD_DIR/simt_cross_core_atomic_kernel.o"
HOST_BINARY="$BUILD_DIR/simt_cross_core_atomic_host"
ENTRY="simt_cross_core_atomic_0_mix_aiv"
...
echo "[CHECK] SIMT atomic source closure and full-contention sequence"
if rg -n '#include.*(cross_core_ordinary|ops-nn)' "$PROBE_ROOT" -g '*.h' -g '*.cpp'; then
    echo "SIMT atomic probe must not include cross_core_ordinary or ops-nn source files." >&2
    exit 1
fi
for required in \
    'LAUNCH_BOUND(kMaxThreadCount)' \
    'asc_atomic_cas(cas_cell, cas_initial, desired)' \
    'asc_atomic_add(add_cell, static_cast<uint64_t>(1U))' \
    'cce::dim3{static_cast<uint32_t>(state->control.thread_count), 1U, 1U}'; do
    if ! grep -Fq "$required" "$KERNEL_SOURCE"; then
        echo "SIMT atomic kernel is missing required source contract: $required" >&2
        exit 1
    fi
done
cas_line="$(grep -nF 'const uint64_t cas_observed = asc_atomic_cas' "$KERNEL_SOURCE" | head -n 1 | cut -d: -f1)"
add_line="$(grep -nF 'const uint64_t add_ticket = asc_atomic_add' "$KERNEL_SOURCE" | head -n 1 | cut -d: -f1)"
return_line="$(grep -nF 'add_returns[thread] = add_ticket' "$KERNEL_SOURCE" | cut -d: -f1)"
fence_line="$(grep -nF 'asc_threadfence();' "$KERNEL_SOURCE" | head -n 1 | cut -d: -f1)"
invoke_line="$(grep -nF 'cce::async_invoke<SimtAtomicContention>' "$KERNEL_SOURCE" | cut -d: -f1)"
wait_line="$(grep -nF 'wait_flag(PIPE_V, PIPE_S, EVENT_ID0)' "$KERNEL_SOURCE" | cut -d: -f1)"
scan_line="$(grep -nF 'result.cas_final = LoadDev64' "$KERNEL_SOURCE" | cut -d: -f1)"
if [[ -z "$cas_line" || -z "$add_line" || -z "$return_line" || -z "$fence_line" || -z "$invoke_line" ||
      -z "$wait_line" || -z "$scan_line" ]] ||
   ! (( cas_line < add_line && add_line < return_line && return_line < fence_line && invoke_line < wait_line && wait_line < scan_line )); then
    echo "SIMT atomic source must preserve CAS < add < return stores < fence and invoke < V/S wait < Scalar scan." >&2
    exit 1
fi

echo "[BUILD] CCEC SIMT atomic AIV object"
"$CCEC" "${COMMON_DEVICE_FLAGS[@]}" -o "$DEVICE_OBJECT" "$KERNEL_SOURCE"

echo "[BUILD] CCEC optimized SIMT atomic bitcode"
...
for symbol in "${required_bitcode_symbols[@]}"; do
    if ! grep -Fq "$symbol" "$BITCODE_DUMP"; then
        echo "optimized SIMT atomic bitcode is missing: $symbol" >&2
        exit 1
    fi
done
echo "[CHECK] bitcode contains 2048-thread SIMT launch, GM uint64 CAS/add, return stores, fence and V/S wait"

echo "[BUILD] AIV-only SIMT atomic ELF"
...
if ! awk '$4 == "FUNC" && $5 == "LOCAL" && $3 + 0 > 0 && $NF ~ /SimtAtomicContention.*_simt_entry$/ {count++}
          END {exit count != 1}' <<<"$SYMBOLS"; then
    echo "final ELF must retain exactly one non-empty LOCAL SIMT atomic entry." >&2
    exit 1
fi
...
if [[ "$RELOCATIONS" != *"There are no relocations"* ]]; then
    echo "final SIMT atomic ELF unexpectedly retains relocations." >&2
    exit 1
fi
...
if [[ "$METADATA_OUTPUT" != *"KERNEL_TYPE: MIX_AIV_MAIN"* ||
    ...
    echo "SIMT atomic AIV metadata is not MIX_AIV_MAIN [0:1]:" >&2
    printf '%s\n' "$METADATA_OUTPUT" >&2
    exit 1
fi
echo "[CHECK] ELF exports only AIV Main entry; SIMT atomic entry is local; metadata is MIX_AIV_MAIN [0:1]"

echo "[BUILD] GCC 15 SIMT atomic ACL host ($($GXX15 -dumpfullversion))"
...
if [[ ! -s "$KERNEL_ELF" || ! -x "$HOST_BINARY" ]]; then
    echo "SIMT atomic build did not produce the required kernel and host artifacts." >&2
    exit 1
fi
echo "[BUILD] SIMT atomic CCEC complete"
