...
本文只记录 `simt_cross_core_dag` 已经发生的开发动作、可复现命令、测试结果、
失败证据和提交。未来计划以
...
- 性能不是阶段强制项；一旦给出性能数字，必须同时给出对照、参数、轮数、
  构建身份和结果文件；
- 每个阶段只提交 `simt_cross_core_dag/` 内的文件；
...
- 目录：`tests/atomic_probe/pa_scheduler/simt_cross_core_dag/`；
- 本阶段只建立设计与过程文档，没有写协议或设备实现；
- 本阶段没有修改 `cross_core_ordinary/`、`same_core/`、runtime 或真实 PA 路径；
- 后续代码不 include、link 或运行时加载 `cross_core_ordinary` 源码。
...
- `cross_core_ordinary/common/shared_exec_protocol.h`；
- `cross_core_ordinary/protocol_probe/` 的 CPU、CCEC 与 host oracle；
- `cross_core_ordinary/ccec/kernel.cpp`、`build.sh` 和 `run.sh`；
- `cross_core_ordinary/common/pa_model.h`、`pa_frontend.h`、
  `pa_exec_adapter.h`；
- `cross_core_ordinary/PA调度器分离版实现过程.md`；
- `docs/simt-launch.md`；
...
6. A5 mixed entry 当前按 1 AIC + 2 AIV/物理 block 编译；AIV id 由 block
   和 subblock 展平；
7. 现有 `cross_core_ordinary` 的 CPU 模型、CCEC 混合 ELF、host oracle 和构建产物
...
- 文档引用的 `cross_core_ordinary` 协议、PA model、adapter 和
...
- `common/shared_protocol.h`：独立定义与 `cross_core_ordinary` 数值一致的
...
所有新增源码都位于 `simt_cross_core_dag/`。构建脚本扫描 C/C++ include，确认
没有 include `cross_core_ordinary` 或本机 `ops-nn` 源码；PTO metadata、CCEC builtin、
...
```bash
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh build-s0
```
...
timeout --foreground 180s \
  tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh \
...
所有源文件仍只位于 `simt_cross_core_dag/`。构建脚本扫描 include，确认没有引用
`cross_core_ordinary` 或 `ops-nn` 源码；PTO/CANN 只作为已安装工具链头使用。
...
```bash
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh build-s1
```
...
guard 隔离。所有源码仍位于 `simt_cross_core_dag/`；构建门槛确认没有 include
`cross_core_ordinary` 或本机 `ops-nn` 源码。
...
```bash
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh build-s2
```
...
所有文件仍位于 `simt_cross_core_dag/`。S3 复用 `gm_probe_support.h` 的 control、
fatal、SIMT report、role result、identity、计数和 checksum 基础函数，没有复制
第三套公共诊断 ABI，也没有 include `cross_core_ordinary` 或本机 `ops-nn` 源码。
...
```bash
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh build-s3
```
...
```bash
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh build-atomic
```
...
`cross_core_ordinary` 或 `ops-nn` 源码依赖。
...
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh build-s4
```
...
```bash
timeout 60s tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh \
  run-s4 --device 0 --runs 1
timeout 120s tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh \
...
```bash
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh build-warp
```
...
`simt_cross_core_dag` 内独立闭合，不 include `cross_core_ordinary` 或 `ops-nn` 源码；但
...
```bash
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh build-g0
```
...
- 源码闭包不 include `cross_core_ordinary`/`ops-nn`；发射 2048 thread、64 warp，且
...
task-submit --timeout 600 --max-time 600 --device auto --device-num 1 \
  --run "cd $PWD && tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh \
...
```bash
timeout --foreground 120s tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh \
  run-g0 --device 0 --batches 1 --runs 10
timeout --foreground 180s tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh \
...
```bash
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh build-g1
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh run-g1 \
...
```bash
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh build-u0
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh run-u0 --device 0 --runs 100
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh build-g1
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh run-g0 --device 0 --batches 1 --runs 1
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh run-g0 --device 0 --batches 256 --runs 1
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh run-g1 --device 0 --batches 1 --runs 1
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh run-g1 --device 0 --batches 256 --runs 1
```
...
```bash
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh build-u1
```
...
```bash
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh build-g0-swimlane
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh \
  run-g0-swimlane --device 0 --batches 256 --runs 1 \
  --swimlane-json \
  tests/atomic_probe/pa_scheduler/simt_cross_core_dag/test_record/2026-8-6/\
...
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh build-g1-swimlane
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh \
...
```bash
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh build-u2
tests/atomic_probe/pa_scheduler/simt_cross_core_dag/run.sh \
...
用户检查新图时指出 AIV task 只有约 5 us，而此前 cross_core_ordinary 的 SF 应约
...
本轮在 SIMT 自有 model 中冻结同一组 `6/28/4/1` 常量，没有引入对 cross_core_ordinary
...
展示方式也按 cross_core_ordinary 的物理 block 合同重做，不再把 96 条 Scalar 和
...
60.233 us，尚不等价于 cross_core_ordinary 的 `TracePhase::Kernel`。最终版把
...
最终口径统一为 cross_core_ordinary 正式 real-compute 默认值：QK/SF/PV/UP 分别执行
