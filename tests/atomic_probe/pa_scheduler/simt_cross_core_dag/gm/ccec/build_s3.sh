...
KERNEL_SOURCE="$SCRIPT_DIR/s3_dual_task_kernel.cpp"
HOST_SOURCE="$SCRIPT_DIR/s3_dual_task_host.cpp"
AIC_OBJECT="$BUILD_DIR/simt_cross_core_s3_aic.o"
AIV_OBJECT="$BUILD_DIR/simt_cross_core_s3_aiv.o"
AIC_BITCODE="$BUILD_DIR/simt_cross_core_s3_aic.bc"
AIV_BITCODE="$BUILD_DIR/simt_cross_core_s3_aiv.bc"
AIC_BITCODE_DUMP="$BUILD_DIR/simt_cross_core_s3_aic.bc.dump"
AIV_BITCODE_DUMP="$BUILD_DIR/simt_cross_core_s3_aiv.bc.dump"
KERNEL_ELF="$BUILD_DIR/simt_cross_core_s3_kernel.o"
HOST_BINARY="$BUILD_DIR/simt_cross_core_s3_host"
AIC_ENTRY="simt_cross_core_s3_0_mix_aic"
AIV_ENTRY="simt_cross_core_s3_0_mix_aiv"
...
echo "[CHECK] S3 source closure and dual-task publication/drain order"
if rg -n '#include.*(cross_core_ordinary|ops-nn)' "$SIMT_ROOT" -g '*.h' -g '*.cpp'; then
    echo "S3 must not include cross_core_ordinary or ops-nn source files." >&2
    exit 1
fi
...
async_line="$(grep -nF 'cce::async_invoke<S3SimtBuildTasks>' "$KERNEL_SOURCE" | cut -d: -f1)"
builder_wait_line="$(awk '/cce::async_invoke<S3SimtBuildTasks>/ {inside=1} inside && /wait_flag\(PIPE_V, PIPE_S, EVENT_ID0\)/ {print NR; exit}' "$KERNEL_SOURCE")"
...
if [[ -z "$payload_line" || -z "$publish_fence_line" || -z "$publish_cas_line" || -z "$async_line" ||
   ...
   ! (( payload_line < publish_fence_line && publish_fence_line < publish_cas_line &&
    ...
    echo "S3 must publish complete payloads before BUILT and finish the SIMT invoke before builder_finished." >&2
    exit 1
fi
...
if [[ -z "$done_cas_line" || -z "$done_count_line" || -z "$vector_call_line" || -z "$vector_complete_line" ||
      -z "$cube_call_line" || -z "$cube_complete_line" ]] ||
   ! (( done_cas_line < done_count_line && vector_call_line < vector_complete_line &&
         cube_call_line < cube_complete_line )); then
    echo "S3 must finish each workload before DONE and increment done_count only after DONE." >&2
    exit 1
fi

echo "[BUILD] CCEC S3 AIC Cube executor (dav-c310-cube)"
"$CCEC" "${COMMON_DEVICE_FLAGS[@]}" --cce-aicore-arch=dav-c310-cube \
    -o "$AIC_OBJECT" "$KERNEL_SOURCE"

echo "[BUILD] CCEC S3 AIV SIMT builder/Vector executor (dav-c310-vec)"
"$CCEC" "${COMMON_DEVICE_FLAGS[@]}" --cce-aicore-arch=dav-c310-vec \
    -o "$AIV_OBJECT" "$KERNEL_SOURCE"

echo "[BUILD] CCEC S3 optimized bitcode inventory"
"$CCEC" "${COMMON_DEVICE_FLAGS[@]}" --cce-aicore-arch=dav-c310-cube \
...
for symbol in "${required_aiv_symbols[@]}"; do
    if ! grep -Fq "$symbol" "$AIV_BITCODE_DUMP"; then
        echo "optimized S3 AIV bitcode is missing: $symbol" >&2
        exit 1
    fi
done
...
for symbol in "${required_aic_symbols[@]}"; do
    if ! grep -Fq "$symbol" "$AIC_BITCODE_DUMP"; then
        echo "optimized S3 AIC bitcode is missing: $symbol" >&2
        exit 1
    fi
done
echo "[CHECK] bitcode contains two-thread SIMT publication, Vector add, Cube matmul and drain atomics"

echo "[BUILD] Static S3 1:2 mixed AICore ELF"
...
for entry in "$AIC_ENTRY" "$AIV_ENTRY"; do
    if ! awk -v name="$entry" \
        ...
        echo "final S3 ELF must define one non-empty GLOBAL entry: $entry" >&2
        exit 1
    fi
    if [[ "$SECTIONS" != *".ascend.meta.$entry"* ]]; then
        echo "final S3 ELF is missing metadata: .ascend.meta.$entry" >&2
        exit 1
    fi
done
...
if [[ "$global_functions" != "$expected_global_functions" ]]; then
    echo "final S3 ELF exports unexpected GLOBAL functions:" >&2
    printf '%s\n' "$global_functions" >&2
    exit 1
fi
for local_pattern in 'S3SimtBuildTasks.*_simt_entry$' 'RunVectorAdd' 'RunCubeMatmul'; do
    if ! awk -v pattern="$local_pattern" \
        '$4 == "FUNC" && $5 == "LOCAL" && $3 + 0 > 0 && $NF ~ pattern {count++}
         END {exit count != 1}' <<<"$SYMBOLS"; then
        echo "final S3 ELF must retain one non-empty LOCAL function matching: $local_pattern" >&2
        exit 1
    fi
done
undefined_globals="$(awk '$5 == "GLOBAL" && $7 == "UND" {print $NF}' <<<"$SYMBOLS")"
if [[ -n "$undefined_globals" ]]; then
    echo "final S3 ELF contains undefined GLOBAL symbols:" >&2
    printf '%s\n' "$undefined_globals" >&2
    exit 1
fi
if [[ "$RELOCATIONS" != *"There are no relocations"* ]]; then
    echo "final S3 ELF unexpectedly retains relocations." >&2
    exit 1
fi
...
if [[ "$(grep -Fc 'KERNEL_TYPE: MIX_AIC_MAIN' <<<"$METADATA_OUTPUT")" -ne 2 ||
    ...
    echo "S3 mixed metadata is not an exact pair of MIX_AIC_MAIN [1:2] entries:" >&2
    printf '%s\n' "$METADATA_OUTPUT" >&2
    exit 1
fi
...
if [[ "$AIV_META_HEX" != *"0c000400 04000000"* || "$AIV_META_HEX" != *"07000400 00200000"* ]]; then
    echo "S3 AIV metadata must encode SIMD_SIMT_MIX_VF=4 and 8 KiB SIMT share memory." >&2
    printf '%s\n' "$AIV_META_HEX" >&2
    exit 1
fi
...
if (( VECTOR_UB_BYTES + SIMT_SHARE_BYTES > MAX_LOCAL_BYTES )); then
    echo "S3 Vector UB plus SIMT share memory exceeds the 224 KiB A5 budget." >&2
    exit 1
fi
echo "[CHECK] ELF has two mixed entries, local SIMT/Vector/Cube functions and 200/224 KiB AIV budget"

echo "[BUILD] GCC 15 S3 ACL host ($("$GXX15" -dumpfullversion))"
...
if [[ ! -s "$KERNEL_ELF" || ! -x "$HOST_BINARY" ]]; then
    echo "S3 build did not produce the required kernel and host artifacts." >&2
    exit 1
fi

echo "[BUILD] S3 CCEC complete"
