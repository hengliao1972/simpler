...
__simt_callee__ __aicore__ __attribute__((always_inline)) inline __gm__ uint64_t *SimtTraceReserveRecord(
    ...
    return reinterpret_cast<__gm__ uint64_t *>(&records[slot]);
}

__simt_callee__ __aicore__ __attribute__((always_inline)) inline void SimtTraceWriteDescriptor(
    __gm__ uint64_t *words, uint32_t task_id, uint16_t site,
    g0_swimlane::TraceKind kind, uint8_t op, uint32_t flags, uint32_t call_count
) {
    ...
    // TraceRecord 为 32B 对齐，words+2 天然 16B 对齐。使用 CANN
    // 已声明的 ulonglong2 store 一次发布 metadata+attributes，
    // 不改变 raw ABI，也不让这些值跨越 atomic/CLOCK64。
    asc_stcg(
        reinterpret_cast<__gm__ ulonglong2 *>(words + 2U),
        make_ulonglong2(
            static_cast<unsigned long long>(task_id) |
                (static_cast<unsigned long long>(encoded_kind_site) << 32U),
            static_cast<unsigned long long>(flags) |
                (static_cast<unsigned long long>(call_count) << 32U)
        )
    );
}
...
__simt_callee__ __aicore__ __attribute__((always_inline)) inline void SimtTraceWriteTimeEndpoints(
    ...
    asc_stcg(
        reinterpret_cast<__gm__ ulonglong2 *>(words),
        make_ulonglong2(
...
__simt_callee__ __aicore__ __attribute__((always_inline)) inline uint64_t SimtTraceAtomicAdd(
    ...
    __gm__ uint64_t *trace_words = SimtTraceReserveRecord(records, counters);
    SimtTraceWriteDescriptor(
        trace_words, task_id, static_cast<uint16_t>(site), g0_swimlane::TraceKind::Atomic,
        static_cast<uint8_t>(op), result_used ? g0_swimlane::kAtomicResultUsed : 0U, 1U
    );
...
__simt_callee__ __aicore__ __attribute__((always_inline)) inline uint64_t SimtTraceAtomicCas(
    ...
    SimtTraceWriteDescriptor(
        ...
        result_used ? g0_swimlane::kAtomicResultUsed : 0U, 1U
    );
    const uint64_t begin = SimtTraceClock();
...
__simt_callee__ __aicore__ __attribute__((always_inline)) inline bool SimtCommitTask(
    ...
    *predecessor_observed = -1;
    for (uint32_t writer = 0U; writer < writer_count; ++writer) {
        const uint32_t symbol_key = static_cast<uint32_t>(history[2U + writer]);
        const uint32_t predecessor_task = static_cast<uint32_t>(history[2U + writer] >> 32U);
        uint32_t producer = 0U;
        uint32_t output_slot = 0U;
        ...
        for (uint32_t earlier = 0U; earlier < writer; ++earlier) {
            const uint32_t earlier_key = static_cast<uint32_t>(history[2U + earlier]);
            const uint32_t earlier_predecessor = static_cast<uint32_t>(history[2U + earlier] >> 32U);
            uint32_t earlier_producer = 0U;
            uint32_t earlier_slot = 0U;
            if (!SimtDecodeSharedSymbolKey(earlier_key, &earlier_producer, &earlier_slot)) {
                SimtPublishFatal(
                    fatal, ExecFatalReason::InsertProtocolFailed, build_owner, task_id G0_SIMT_TRACE_ARGUMENT
                );
                return false;
