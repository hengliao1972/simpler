...
#if PTO_FDWIC_SHARED_MAP
// Host 只负责把一块已经分配好的外置 cell 数组描述给 AICPU/AICore。
// 本 helper 不初始化、不清零、更不生成任何 task identity 或 payload；
// producer 对 cell 的首次 ordinary store 只能发生在 AICPU 路径。
// capacity 是运行期已分配槽数，可以小于下游物理表上限，但绝不能超过
// kRuntimePlanConsumerCapacity，否则合法 Plan task_id 可能越界访问
// shared_outputs/claim_tournament/exec_cells 等 task-indexed 消费者。
inline bool ConfigureRuntimePlanStorageRef(
    aicpu_plan::RuntimePlanStorageRef *storage,
    void *aligned_cells, uint32_t capacity
) {
    if (storage == nullptr) {
        return false;
    }

    // 失败时始终留下全零的无效描述，避免调用者误用上一轮地址。
    std::memset(storage, 0, sizeof(*storage));
    if (aligned_cells == nullptr || capacity == 0U ||
        capacity > kRuntimePlanConsumerCapacity) {
        return false;
    }

    const uintptr_t base = reinterpret_cast<uintptr_t>(aligned_cells);
    if ((base % aicpu_plan::kAtomicIsolationBytes) != 0U) {
        return false;
    }

    constexpr uint64_t kCellBytes =
        sizeof(aicpu_plan::RuntimeTaskPlanCell);
    if (static_cast<uint64_t>(capacity) > UINT64_MAX / kCellBytes) {
        return false;
    }
    const uint64_t cells_bytes =
        static_cast<uint64_t>(capacity) * kCellBytes;
    const uint64_t base64 = static_cast<uint64_t>(base);
    if (base64 > UINT64_MAX - cells_bytes) {
        return false;
    }

    aicpu_plan::RuntimePlanStorageRef candidate{};
    candidate.cells_base = base64;
    candidate.cells_bytes = cells_bytes;
    candidate.capacity = capacity;
    candidate.cell_bytes =
        sizeof(aicpu_plan::RuntimeTaskPlanCell);
    candidate.abi_version = aicpu_plan::kRuntimePlanAbiVersion;
    if (!aicpu_plan::RuntimePlanStorageRefValid(candidate)) {
        return false;
    }
    *storage = candidate;
    return true;
...
inline bool ConfigureRuntimePlanStorage(
    SchedulerState *state, void *aligned_cells, uint32_t capacity
) {
    if (state == nullptr) {
        return false;
    }
    return ConfigureRuntimePlanStorageRef(
        &state->runtime_plan_storage, aligned_cells, capacity
    );
}
#endif
...
inline void InitializeState(SchedulerState *state, const Options &options) {
    ...
    for (uint32_t task_id = 0; task_id < kMaxTasks; ++task_id) {
        ...
        // 值并充当未触碰 canary；正式逐 task completion
    ...
    std::memset(
        &state->runtime_plan_control, 0,
        sizeof(state->runtime_plan_control)
    );
    std::memset(
        &state->runtime_plan_storage, 0,
...
static_assert(
    ...
    CrossCoreExecStateBytes() ==
        19441088 + sizeof(aicpu_plan::RuntimePlanControl) +
            sizeof(aicpu_plan::RuntimePlanStorageRef),
...
inline bool ExpandSharedTraceRecords(
    ...
    for (uint32_t task_id = 0;
        ...
        claim.flags =
            (winner ? kClaimWon : 0U) | kClaimAttempted;
        claim.phase =
            static_cast<uint16_t>(TracePhase::Claim);
    ...
    const bool count_closed = logical_records->size() ==
        static_cast<size_t>(generic_count) +
            2U * plan.total_tasks;
    if (!count_closed) {
        std::fprintf(
            stderr,
            "shared trace replay count mismatch: worker=%u "
            "generic=%u expected_submits=%u "
            "logical=%zu expected_logical=%zu.\n",
            worker, generic_count, plan.total_tasks,
...
inline Metrics Validate(
    ...
    uint32_t runtime_executable_task_count = 0;
    ...
    const uint64_t oracle_exec_completions =
        shared_plan_ok
        ...
        : 0;
    // 执行闭合数必须从 device 实际发布的终态 cell 推导，不能再读取
    // Host 预制计划曾写入的 executable_task_count。Host plan 只作为
    // 独立业务 oracle，额外证明 runtime cell 数没有少执行或多执行。
    cross_core_exec_drain_ok &=
        runtime_executable_task_count == oracle_exec_completions &&
        cross_core_exec_drain_completions ==
            runtime_executable_task_count;
    ...
    // insert_completion 表示严格 task-id 插入链的完成，不是
    // “本 task 是否有 metadata writer”的位图。因此空 writer task
    // 也必须从 N-1 推进为 N，否则 N+1 永远不能进入有序插入段。
    bool shared_insert_completions_ok = true;
    bool legacy_task_completion_canary_ok = true;
    uint32_t shared_completed_insert_tasks = 0;
    for (uint32_t task_id = 0; task_id < task_count; ++task_id) {
        const SharedHostPlannedTask *planned_task =
            shared_plan.TaskAt(task_id);
    ...
    Expect(
        shared_insert_completions_ok,
        "every shared task advances its strict insert-completion word and unused tasks stay pending",
        &metrics
    );
    Expect(
        shared_runtime_plan_control_ok &&
