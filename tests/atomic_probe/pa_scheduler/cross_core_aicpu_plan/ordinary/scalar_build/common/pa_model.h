...
// Runtime Plan 的外置 cell 只负责给下游 task-indexed 状态提供身份与参数。
// 因而它的生产容量上限必须与这些消费者的物理容量相同，不能因为公共
// Plan ABI 允许 32K task 就让 producer 发布下游无法寻址的 task_id。
// 该常量描述通用消费者容量；4352 只是当前 shared standalone 布局的
// 编译期事实，不是 Host 根据 batches 或 PA 公式计算出的本轮 task 数。
constexpr uint32_t kRuntimePlanConsumerCapacity = kMaxTasks;
static_assert(
    kRuntimePlanConsumerCapacity > 0U &&
        kRuntimePlanConsumerCapacity <= aicpu_plan::kMaxRuntimeTasks,
    "runtime Plan consumers exceed the common Plan ABI capacity"
);
#else
...
struct alignas(64) SharedTensorMapSidecar {
    ...
    SharedOutputCell shared_outputs[kRuntimePlanConsumerCapacity];
    ...
    SharedWriterHistoryCell writer_history[kRuntimePlanConsumerCapacity];
...
struct alignas(128) SchedulerState {
    ...
    SharedClaimTournamentTask
        claim_tournament[kRuntimePlanConsumerCapacity];
    ...
    cross_core::SharedExecCell exec_cells[kRuntimePlanConsumerCapacity];
    ...
    // AICPU 是唯一 logical Plan producer。descriptor ordinary payload 与
    // 六条 128B atomic-only 控制线完全分离；Build backend 只从这份
    // runtime canonical Plan 取得 task identity，不再读取 Host 答案。
    aicpu_plan::RuntimePlanControl runtime_plan_control;
    // PlanCell 最大约 4.5 KiB，按本轮容量单独分配；不能继续膨胀已经约
    // 1 GiB 的 SchedulerState。Host 只写地址、容量和 ABI，不写 task。
    aicpu_plan::RuntimePlanStorageRef runtime_plan_storage;
#endif
};
...
static_assert(
    ...
    "shared-output capacity diverged from runtime Plan consumers"
);
static_assert(
    sizeof(SharedTensorMapSidecar::writer_history) /
            sizeof(SharedWriterHistoryCell) ==
        kRuntimePlanConsumerCapacity,
    "writer-history capacity diverged from runtime Plan consumers"
);
static_assert(
    sizeof(SchedulerState::claim_tournament) /
            sizeof(SharedClaimTournamentTask) ==
        kRuntimePlanConsumerCapacity,
    "Claim completion capacity diverged from runtime Plan consumers"
);
static_assert(
    sizeof(SchedulerState::exec_cells) /
            sizeof(cross_core::SharedExecCell) ==
        kRuntimePlanConsumerCapacity,
    "Execute-cell capacity diverged from runtime Plan consumers"
);
static_assert(
...
static_assert(
    ...
    "AICPU runtime Plan control must follow Execute cursors"
);
static_assert(
    offsetof(SchedulerState, runtime_plan_storage) ==
        offsetof(SchedulerState, runtime_plan_control) +
            sizeof(aicpu_plan::RuntimePlanControl),
    "AICPU runtime Plan storage reference must follow Plan control"
);
static_assert(
    alignof(SchedulerState) == aicpu_plan::kAtomicIsolationBytes,
    "SchedulerState must preserve the 128B Plan control alignment"
);
static_assert(
    offsetof(SchedulerState, runtime_plan_control) %
                aicpu_plan::kAtomicIsolationBytes == 0U &&
        offsetof(SchedulerState, runtime_plan_storage) %
                aicpu_plan::kAtomicIsolationBytes == 0U,
    "runtime Plan control and storage reference must be 128B aligned"
);
static_assert(
    offsetof(SchedulerState, runtime_plan_storage) +
            sizeof(aicpu_plan::RuntimePlanStorageRef) ==
        sizeof(SchedulerState),
    "runtime Plan storage reference must remain the SchedulerState tail"
);
static_assert(
    sizeof(SchedulerState) % aicpu_plan::kAtomicIsolationBytes == 0U,
    "SchedulerState tail must close on a 128B boundary"
);
static_assert(
        offsetof(SchedulerState, exec_fatal) %
        ...
        offsetof(SchedulerState, exec_scan_cursors) %
                cross_core::kExecCacheLineBytes == 0 &&
        offsetof(SchedulerState, runtime_plan_control) %
                aicpu_plan::kPlanCacheLineBytes == 0 &&
        offsetof(SchedulerState, runtime_plan_storage) %
                aicpu_plan::kPlanCacheLineBytes == 0,
