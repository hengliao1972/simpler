...
class ScopedAlignedAclDeviceAllocation {
public:
    ScopedAlignedAclDeviceAllocation() = default;
    ScopedAlignedAclDeviceAllocation(const ScopedAlignedAclDeviceAllocation &) = delete;
    ScopedAlignedAclDeviceAllocation &operator=(const ScopedAlignedAclDeviceAllocation &) = delete;

    ~ScopedAlignedAclDeviceAllocation() {
        // 只能释放 aclrtMalloc 返回的原始地址，不能释放向上取整后的内部地址。
        // 正常路径会先 ReleaseRaw 并检查错误；任一早退路径由析构兜底。
        if (raw_pointer_ != nullptr) (void)aclrtFree(raw_pointer_);
    }

    bool Allocate(size_t payload_bytes, size_t alignment, const char *label) {
        if (raw_pointer_ != nullptr || payload_bytes == 0U || alignment == 0U ||
            (alignment & (alignment - 1U)) != 0U ||
            payload_bytes > std::numeric_limits<size_t>::max() - (alignment - 1U)) {
            std::fprintf(stderr, "Invalid aligned device allocation request: %s\n", label);
            return false;
        }
        const size_t raw_bytes = payload_bytes + alignment - 1U;
        void *raw = nullptr;
        if (!CheckAcl(
                aclrtMalloc(&raw, raw_bytes, ACL_MEM_MALLOC_HUGE_FIRST),
                label
            )) {
            return false;
        }
        raw_pointer_ = raw;
        const uintptr_t raw_address = reinterpret_cast<uintptr_t>(raw_pointer_);
        if (raw_address > std::numeric_limits<uintptr_t>::max() - (alignment - 1U)) {
            std::fprintf(stderr, "Device address overflows while aligning: %s\n", label);
            return false;
        }
        const uintptr_t aligned_address =
            (raw_address + alignment - 1U) & ~(static_cast<uintptr_t>(alignment) - 1U);
        aligned_pointer_ = reinterpret_cast<void *>(aligned_address);
        return true;
    }

    void *GetAligned() const { return aligned_pointer_; }

    void *ReleaseRaw() {
        void *pointer = raw_pointer_;
        raw_pointer_ = nullptr;
        aligned_pointer_ = nullptr;
        return pointer;
    }

private:
    void *raw_pointer_ = nullptr;
    void *aligned_pointer_ = nullptr;
};
...
int main(int argc, char **argv) {
    ...
    // 内部对齐地址；释放时仍保存并释放原始地址。
    ScopedAlignedAclDeviceAllocation state_allocation;
    if (!state_allocation.Allocate(
            sizeof(pa_scheduler::SchedulerState),
            pa_scheduler::aicpu_plan::kAtomicIsolationBytes,
            "aclrtMalloc(state with 128-byte alignment slack)"
        )) {
        return EXIT_FAILURE;
    }
    void *state_device = state_allocation.GetAligned();
    if ((reinterpret_cast<uintptr_t>(state_device) &
         (pa_scheduler::aicpu_plan::kAtomicIsolationBytes - 1U)) != 0U) {
        std::fprintf(stderr, "Device state is not 128-byte aligned: %p\n", state_device);
        return EXIT_FAILURE;
    }

#if PTO_FDWIC_SHARED_MAP
    // Plan 槽容量只能来自 AICore 编译期消费者上限，不能由 Host 的 PA
    // batch/context oracle 推导。Host 只分配和清零这块 GM，不生成 task
    // identity/payload；正式内容只能由后续 AICPU producer 发布。
    constexpr uint32_t kRuntimePlanCapacity =
        pa_scheduler::kRuntimePlanConsumerCapacity;
    static_assert(
        static_cast<size_t>(kRuntimePlanCapacity) <=
            std::numeric_limits<size_t>::max() /
                sizeof(pa_scheduler::aicpu_plan::RuntimeTaskPlanCell),
        "external RuntimeTaskPlanCell allocation must fit size_t"
    );
    constexpr size_t kRuntimePlanCellsBytes =
        static_cast<size_t>(kRuntimePlanCapacity) *
        sizeof(pa_scheduler::aicpu_plan::RuntimeTaskPlanCell);
    ScopedAlignedAclDeviceAllocation runtime_plan_allocation;
    if (!runtime_plan_allocation.Allocate(
            kRuntimePlanCellsBytes,
            pa_scheduler::aicpu_plan::kAtomicIsolationBytes,
            "aclrtMalloc(RuntimeTaskPlanCell storage with 128-byte alignment slack)"
        )) {
        return EXIT_FAILURE;
    }
    void *runtime_plan_cells_device =
        runtime_plan_allocation.GetAligned();
    if ((reinterpret_cast<uintptr_t>(runtime_plan_cells_device) &
         (pa_scheduler::aicpu_plan::kAtomicIsolationBytes - 1U)) != 0U) {
        std::fprintf(
            stderr,
            "RuntimeTaskPlanCell storage is not 128-byte aligned: %p\n",
    ...
    for (uint32_t run = 1; run <= options.runs; ++run) {
        ...
        // 写入本轮 SchedulerState。这里不拷贝 Host task plan，也不按本轮
        // batch 数缩小容量；AICPU producer 将在独立阶段填入真实 callback
        // 产生的 canonical payload。
        if (!CheckAcl(
                aclrtMemset(
                    runtime_plan_cells_device,
                    kRuntimePlanCellsBytes,
                ...
                "aclrtMemset(RuntimeTaskPlanCell storage)"
            ) ||
            !pa_scheduler::host::ConfigureRuntimePlanStorage(
                state.get(), runtime_plan_cells_device,
                kRuntimePlanCapacity
            )) {
            std::fprintf(
                stderr,
                "Failed to configure the external RuntimeTaskPlanCell storage.\n"
            );
            execution_ok = false;
            all_passed = false;
            break;
...
#endif
    cleanup_ok &= CheckAcl(
        aclrtFree(state_allocation.ReleaseRaw()),
        "aclrtFree(state raw allocation)"
    );
