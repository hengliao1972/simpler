...
struct CcecOps {
    ...
    // 发布 release 都必须走返回型原子指令，绝不能对该行执行 DCCI。
    // LoadControl 返回 atomic 已完成且旧值可被本核消费的边界。
    __aicore__ static inline int64_t LoadControl(
    ...
    // FetchAddControl 的返回值是递增前旧值；中央 build ticket 与 worker
    // arrival 都直接消费它，不能改成只发射、不等待返回的原子写。
    __aicore__ static inline int64_t FetchAddControl(
        __gm__ volatile int64_t *address, int64_t value
    ) {
        return atomicAdd(const_cast<__gm__ int64_t *>(address), value);
    }

    // Scalar 只允许用 Exchange 发布自身拥有的 Plan control。atomic-only
    // 行没有 ordinary dirty payload，因此这里不做 FlushRegion/DCCI；调用
    // 者若要发布普通 payload，必须先按协议单独 clean payload 区域。
    __aicore__ static inline void PublishControl(
        __gm__ volatile int64_t *address, int64_t value
    ) {
        (void)atomicExch(const_cast<__gm__ int64_t *>(address), value);
    }
