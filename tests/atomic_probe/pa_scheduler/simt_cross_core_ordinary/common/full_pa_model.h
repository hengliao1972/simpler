...
#ifndef PA_SCHEDULER_SIMT_CROSS_CORE_ORDINARY_FULL_PA_MODEL_H
#define PA_SCHEDULER_SIMT_CROSS_CORE_ORDINARY_FULL_PA_MODEL_H
...
#define SIMT_CROSS_CORE_G0_BUILDER_WARP_COUNT 5U
#endif
...
#ifndef SIMT_CROSS_CORE_G0_DISPATCH_WINDOW_BATCHES
#define SIMT_CROSS_CORE_G0_DISPATCH_WINDOW_BATCHES 256U
#endif
// 同一窗口内先派发上游 task，再派发下游 task；窗口大小只控制通用
// topological wave 的深度，不改变 task、依赖或 workload。
constexpr uint32_t kDispatchWindowBatches = SIMT_CROSS_CORE_G0_DISPATCH_WINDOW_BATCHES;
static_assert(
    kDispatchWindowBatches >= 1U && kDispatchWindowBatches <= kDefaultBatches,
    "dispatch window must remain in 1..256 batches"
);
...
SIMT_CROSS_CORE_G0_MODEL_INLINE uint32_t AicDispatchTaskId(uint32_t index, uint32_t batch_count) {
    const uint32_t window = batch_count < kDispatchWindowBatches ? batch_count : kDispatchWindowBatches;
    const uint32_t group_span = 2U * window;
    const uint32_t group_begin = (index / group_span) * window;
    const uint32_t group_count = batch_count - group_begin < window ? batch_count - group_begin : window;
    const uint32_t within_group = index % group_span;
    const bool upstream = within_group < group_count;
    const uint32_t batch = group_begin + (upstream ? within_group : within_group - group_count);
    return BatchTaskId(batch, upstream ? TaskKind::Qk : TaskKind::Pv);
}

SIMT_CROSS_CORE_G0_MODEL_INLINE uint32_t AivDispatchTaskId(uint32_t index, uint32_t batch_count) {
    const uint32_t window = batch_count < kDispatchWindowBatches ? batch_count : kDispatchWindowBatches;
    const uint32_t group_span = 2U * window;
    const uint32_t group_begin = (index / group_span) * window;
    const uint32_t group_count = batch_count - group_begin < window ? batch_count - group_begin : window;
    ...
    const bool upstream = within_group < group_count;
    const uint32_t batch = group_begin + (upstream ? within_group : within_group - group_count);
    return BatchTaskId(batch, upstream ? TaskKind::Sf : TaskKind::Up);
}
...
#endif  // PA_SCHEDULER_SIMT_CROSS_CORE_ORDINARY_FULL_PA_MODEL_H
