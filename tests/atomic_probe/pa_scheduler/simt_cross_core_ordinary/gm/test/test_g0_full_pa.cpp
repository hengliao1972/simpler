...
public:
    CpuFullPaModel(uint32_t batches, uint64_t nonce, uint32_t builder_count) :
        ...
        for (uint32_t index = 0U; index < 2U * batches_; ++index) {
            aic_task_ids_.push_back(AicDispatchTaskId(index, batches_));
            aiv_task_ids_.push_back(AivDispatchTaskId(index, batches_));
        }
...
private:
    void InitializeTask(uint32_t task_id) {
        ...
        if (TaskExecutable(kind)) {
            // Immutable witness identity is launch input.  Dynamic evidence is
            // still zero and can only become valid through task execution.
            task.witness_nonce = nonce_;
            task.witness_magic = kExecutionWitnessMagic;
            task.witness_task = task_id;
            task.witness_kind = kind;
            task.witness_owner = UINT32_MAX;
        }
    ...
    bool RunBuilderWarp(uint32_t thread_id) {
        ...
        for (uint32_t task_id = first_attempt; task_id < task_count_;
            ...
            aggregate_reserve += AlignOutputBytes(TaskOutputBytes(TaskKindAt(task_id)));
        }
        uint64_t aggregate_cursor = heap_vend_.fetch_add(aggregate_reserve, std::memory_order_acq_rel);
        if (aggregate_reserve > kHeapBytes || aggregate_cursor > kHeapBytes - aggregate_reserve) {
            PublishFatal(ExecFatalReason::HeapReservationFailed, first_attempt, owner);
            return false;
        }
        for (uint32_t task_id = first_attempt; task_id < task_count_;
             task_id += BuilderLeaderCount(builder_count_)) {
            ++report.task_state_access_count;
            const BuildAttemptResult result = BuildTask(task_id, thread_id, &aggregate_cursor);
    ...
    bool ReserveHeap(uint32_t task_id, uint64_t *aggregate_cursor, uint64_t *task_base, uint64_t *vend) {
        const uint64_t reserve = AlignOutputBytes(TaskOutputBytes(TaskKindAt(task_id)));
        if (reserve == 0U) {
            *task_base = 0U;
            *vend = *aggregate_cursor;
        } else {
    ...
    BuildAttemptResult BuildTask(uint32_t task_id, uint32_t thread_id, uint64_t *aggregate_cursor) {
        CpuTask &task = tasks_[task_id];
        const TaskKind kind = TaskKindAt(task_id);
        ...
        uint64_t vend = 0U;
        if (!ReserveHeap(task_id, aggregate_cursor, &task_base, &vend)) {
            return BuildAttemptResult::Error;
        }
        ...
        const bool publishes_metadata = MetadataInsertContractPresent(metadata_insert_contract);
        const uint32_t writer_count = MetadataInsertWriterCount(metadata_insert_contract);
        const int32_t previous_metadata_writer = MetadataInsertPreviousWriter(metadata_insert_contract);
        if ((metadata_insert_contract & ~kMetadataInsertContractAllowedMask) != 0U ||
            ...
            (publishes_metadata &&
             (writer_count == 0U || writer_count > kWriterHistoryMaxPerTask ||
              previous_metadata_writer != PreviousMetadataWriterTask(task_id) ||
        ...
        if (MetadataInsertContractPresent(metadata_insert_contract)) {
            ...
            // writer_count>0 is guaranteed by the contract.  The globally
            // serialized writer turn makes each derived last_writer location
            // single-writer here; the release store models the device-side
            // non-cacheable store plus the fence before insert_completion.
            for (uint32_t writer = 0U; writer < writer_count; ++writer) {
                ...
                // 全局 ordinary writer predecessor 已建立唯一写入顺序；
                // last_writer 只是该串行插入的派生元数据，不再重复 CAS。
                tasks_[producer].last_writer[output_slot].store(
                    static_cast<int64_t>(task_id), std::memory_order_release
                );
            }
        }
        if (MetadataInsertContractPresent(metadata_insert_contract)) {
        ...
        if (MetadataInsertContractPresent(metadata_insert_contract)) {
            task.insert_completion.store(static_cast<int64_t>(task_id), std::memory_order_release);
        }
        if (kind == TaskKind::Alloc) {
            task.completion_vend.store(vend, std::memory_order_relaxed);
            task.completion_flag.store(1, std::memory_order_release);
        } else {
            task.completion_vend.store(vend, std::memory_order_release);
            task.exec_state.store(BuiltState(task_id, build_owner), std::memory_order_release);
        }
    ...
    bool PublishedPayloadHeaderValid(uint32_t task_id, ExecPayloadLayout *layout_out = nullptr) const {
    ...
    bool PayloadValid(uint32_t task_id) const {
        ...
        const TaskExecShape shape = TaskShape(kind);
        ExecPayloadLayout layout{};
        if (!PublishedPayloadHeaderValid(task_id, &layout)) {
            return false;
        }
        for (uint32_t tensor_index = 0U; tensor_index < shape.tensor_count; ++tensor_index) {
            TensorDesc expected{};
    ...
    bool BindPublishedPayload(ExecutionToken *token, uint32_t task_id, uint32_t owner) const {
        const CpuTask &task = tasks_[task_id];
        const TaskExecShape shape = TaskShape(TaskKindAt(task_id));
        ExecPayloadLayout layout{};
        if (!PublishedPayloadHeaderValid(task_id, &layout)) {
    ...
    bool AdvanceToken(CpuOwner &worker, uint32_t slot) {
        ...
        if (token.control.phase == ExecTokenPhase::WaitingBuilt) {
            ...
            if (!BindPublishedPayload(&token, task_id, worker.result.owner)) {
        ...
        const ExecPayloadHeader header = DecodeCpuHeader(task.payload);
        const uint64_t output_checksum = PackOutputPair(owner_workload_[worker.result.owner][kind_slot]);
        task.witness_state.store(
            ExecutionWitnessState(
                nonce_, task_id, kind, worker.result.owner, 1U,
                kCompletionSequenceWorkloadWitnessVendFlagDone, output_checksum,
                token.control.fanin_ready_prefix
            ),
            std::memory_order_release
        );
        if (task.completion_vend.load(std::memory_order_acquire) != header.completion_vend) {
            PublishFatal(ExecFatalReason::CompletionPublishFailed, task_id, worker.result.owner);
            return false;
        }
        task.completion_flag.store(1, std::memory_order_release);
        task.exec_state.store(
            DoneState(task_id, token.control.build_owner, worker.result.owner), std::memory_order_release
        );
    ...
    bool ValidateCounts() const {
        if (fatal_.load(std::memory_order_acquire) != 0U ||
            ...
            done_count_.load(std::memory_order_acquire) != 0U ||
            alloc_done_.load(std::memory_order_acquire) != 0U ||
            aic_done_.load(std::memory_order_acquire) != 0U ||
            aiv_done_.load(std::memory_order_acquire) != 0U ||
    ...
    bool ValidateCompletionAndWitnesses() const {
        for (uint32_t task_id = 0U; task_id < task_count_; ++task_id) {
            ...
            const DecodedExecState decoded =
                DecodeExecState(static_cast<int64_t>(task.exec_state.load(std::memory_order_acquire)));
            const uint32_t fanin_count = TaskFaninCount(task_id);
            const uint64_t expected_witness = ExecutionWitnessState(
                nonce_, task_id, kind, decoded.execute_owner, 1U,
                kCompletionSequenceWorkloadWitnessVendFlagDone, ExpectedWorkloadOutputPair(kind), fanin_count
            );
            ...
            if (task.witness_state.load(std::memory_order_acquire) != expected_witness ||
                ...
                task.witness_owner != UINT32_MAX || task.execution_count != 0U || task.completion_sequence != 0U ||
                task.output_checksum != 0U || task.witness_fanin_ready_prefix != 0U ||
    ...
    bool ValidateRetainedTokenBinding(const CpuOwner &worker, uint32_t slot, uint32_t expected_task) const {
        ...
        const TaskKind kind = TaskKindAt(task_id);
        const TaskExecShape shape = TaskShape(kind);
        const DecodedExecState decoded = DecodeExecState(
            static_cast<int64_t>(task.exec_state.load(std::memory_order_acquire))
        );
        ExecPayloadLayout layout{};
        if (!TaskExecutable(kind) || TaskEngine(kind) != OwnerEngine(worker.result.owner, builder_count_) ||
            !decoded.valid || decoded.execute_owner != worker.result.owner ||
            task.witness_state.load(std::memory_order_acquire) !=
                ExecutionWitnessState(
                    nonce_, task_id, kind, worker.result.owner, 1U,
                    kCompletionSequenceWorkloadWitnessVendFlagDone, ExpectedWorkloadOutputPair(kind),
                    TaskFaninCount(task_id)
                ) ||
            !ComputeExecPayloadLayout(shape.tensor_count, shape.scalar_count, shape.fanin_count, layout)) {
            return false;
        }
    ...
    bool ValidateRolesAndDrain() const {
        ...
        for (uint32_t task_id = 0U; task_id < task_count_; ++task_id) {
            ...
            if (!decoded.valid || decoded.execute_owner >= kOwnerCount ||
                IsBuilderOwner(decoded.execute_owner, builder_count_)) {
                return false;
            }
            ++witness_by_owner[decoded.execute_owner][static_cast<uint32_t>(kind)];
        }
        for (uint32_t owner = 0U; owner < kOwnerCount; ++owner) {
    ...
    bool ValidateMaterializedToken(const FullPaState &state, uint32_t owner, uint32_t slot) const {
        ...
        const TaskKind kind = TaskKindAt(task_id);
        const TaskExecShape shape = TaskShape(kind);
        const DecodedExecState decoded = DecodeExecState(task.exec.control.state);
        ExecPayloadLayout layout{};
        if (!TaskExecutable(kind) || TaskEngine(kind) != OwnerEngine(owner, builder_count_) ||
            !decoded.valid || decoded.execute_owner != owner ||
            static_cast<uint64_t>(task.execution_witness.state) !=
                ExecutionWitnessState(
                    nonce_, task_id, kind, owner, 1U, kCompletionSequenceWorkloadWitnessVendFlagDone,
                    ExpectedWorkloadOutputPair(kind), TaskFaninCount(task_id)
                ) ||
            !ComputeExecPayloadLayout(shape.tensor_count, shape.scalar_count, shape.fanin_count, layout)) {
            return false;
        }
    ...
    bool ValidateAbiState(const FullPaState &state) const {
        ...
        for (uint32_t index = 0U; index < aic_task_ids_.size(); ++index) {
            if (state.exec_dispatch.aic_task_ids[index] != AicDispatchTaskId(index, batches_)) {
                return false;
            }
        }
        ...
        for (uint32_t index = 0U; index < aiv_task_ids_.size(); ++index) {
            if (state.exec_dispatch.aiv_task_ids[index] != AivDispatchTaskId(index, batches_)) {
                return false;
            }
        }
...
bool TestStaticBuilderPartition() {
    const uint32_t task_counts[] = {kTasksPerBatch, kMainTaskCount};
    for (uint32_t builder_count = 1U; builder_count <= kMaxBuilderCount; ++builder_count) {
        for (uint32_t task_count : task_counts) {
            std::vector<uint8_t> assignments(task_count, 0U);
            for (uint32_t thread = 0U; thread < BuilderThreadCount(builder_count); ++thread) {
                if (!BuilderThreadActive(thread, builder_count)) {
                    continue;
                }
                uint32_t assigned = 0U;
                for (uint32_t task_id = BuilderFirstTask(thread, builder_count); task_id < task_count;
                     task_id += BuilderLeaderCount(builder_count)) {
                    if (BuilderThreadForTask(task_id, builder_count) != thread || assignments[task_id] != 0U) {
                        return false;
                    }
                    assignments[task_id] = 1U;
                    ++assigned;
                }
                if (assigned != BuilderExpectedTaskCount(thread, task_count, builder_count)) {
                    return false;
                }
            }
            if (!std::all_of(assignments.begin(), assignments.end(), [](uint8_t count) { return count == 1U; })) {
                return false;
            }
        }
    }
...
bool TestHalfPacketAndUniqueDispatch() {
    ...
    const bool premature_dispatch = state.load(std::memory_order_acquire) == BuiltState(1U);
    if (premature_dispatch) {
        payload_reads.fetch_add(1U, std::memory_order_relaxed);
    }
    pause.Release();
    builder.join();
    if (premature_dispatch || payload_reads.load(std::memory_order_relaxed) != 0U ||
        state.load(std::memory_order_acquire) != BuiltState(1U)) {
        return false;
    }
    // The production path obtains ownership from a unique fetch_add ticket,
    // then publishes Claimed with a plain store.  Exercise that exact contract
    // with multiple dispatchers: only ticket zero maps to this single task.
    std::atomic<uint32_t> cursor{0U};
    std::atomic<uint32_t> winners{0U};
    std::vector<std::thread> dispatchers;
    for (uint32_t owner = 0U; owner < 8U; ++owner) {
        dispatchers.emplace_back([&, owner] {
            const uint32_t ticket = cursor.fetch_add(1U, std::memory_order_acq_rel);
            if (ticket == 0U) {
                if (state.load(std::memory_order_acquire) != BuiltState(1U)) {
                    payload_reads.fetch_add(1000U, std::memory_order_relaxed);
                    return;
                }
                state.store(ClaimedState(1U, owner), std::memory_order_release);
    ...
    const DecodedExecState decoded = DecodeExecState(static_cast<int64_t>(state.load(std::memory_order_acquire)));
    return cursor.load(std::memory_order_relaxed) == 8U && winners.load(std::memory_order_relaxed) == 1U &&
           payload_reads.load(std::memory_order_relaxed) == payload.size() && decoded.valid &&
           decoded.phase == ExecPhase::Claimed && decoded.task_id == 1U;
}

bool TestTokenWindowWaitingBuilt() {
    std::array<std::atomic<uint64_t>, kTokensPerOwner + 1U> states{};
    std::array<ExecutionToken, kTokensPerOwner> tokens{};
    for (uint32_t task = 0U; task < states.size(); ++task) {
        states[task].store(0U, std::memory_order_relaxed);
    }
    states[kTokensPerOwner - 1U].store(0U, std::memory_order_relaxed);
    ...
    if (busy != kTokensPerOwner || next_ticket != kTokensPerOwner) {
        return false;
    }
    ...
    if (payload_reads != 0U || next_ticket != kTokensPerOwner ||
        states[kTokensPerOwner - 1U].load(std::memory_order_acquire) != 0U ||
        tokens[kTokensPerOwner - 1U].control.phase != ExecTokenPhase::WaitingBuilt) {
        return false;
    }
    const uint64_t empty_observed = states[kTokensPerOwner - 1U].load(std::memory_order_acquire);
    if (empty_observed != 0U ||
        tokens[kTokensPerOwner - 1U].control.phase != ExecTokenPhase::WaitingBuilt) {
        return false;
    }
    states[0].store(BuiltState(1U), std::memory_order_release);
    if (states[0].load(std::memory_order_acquire) != BuiltState(1U)) {
        return false;
    }
    states[0].store(ClaimedState(1U, 0U), std::memory_order_release);
    ...
    ++busy;
    return busy == kTokensPerOwner && next_ticket == kTokensPerOwner + 1U;
}

bool TestBuildExecuteOverlap() {
    std::atomic<uint64_t> task1_state{0U};
    std::atomic<uint64_t> task2_state{0U};
    ...
    const bool claimed = task1_state.load(std::memory_order_acquire) == BuiltState(1U);
    if (claimed) {
        task1_state.store(ClaimedState(1U, 0U), std::memory_order_release);
    }
    const bool later_still_unpublished =
        task2_state.load(std::memory_order_acquire) == 0U &&
        task2_insert.load(std::memory_order_acquire) == InsertCompletionInitialValue(2U);
    task1_flag.store(1, std::memory_order_release);
    const bool completed = task1_state.load(std::memory_order_acquire) == ClaimedState(1U, 0U);
    if (completed) {
        task1_state.store(DoneState(1U, 0U), std::memory_order_release);
    }
...
bool TestOrdinaryGlobalWriterContract() {
    uint32_t writer_tasks = 0U;
    uint32_t writer_records = 0U;
    uint32_t predecessor_waits = 0U;
    int32_t expected_previous_writer = -1;
    for (uint32_t task_id = 0U; task_id < kMainTaskCount; ++task_id) {
        ...
        const uint32_t expected_wait_count = expected_previous_writer < 0 ? 0U : 1U;
        const uint32_t expected_latest =
            expected_previous_writer < 0 ? UINT32_MAX : static_cast<uint32_t>(expected_previous_writer);
        const uint64_t expected_contract =
            kMetadataInsertContractPresent | writer_count |
            (static_cast<uint64_t>(expected_previous_writer + 1) << kMetadataInsertPreviousWriterShift);
        if (contract != expected_contract || (contract & ~kMetadataInsertContractAllowedMask) != 0U ||
            PreviousMetadataWriterTask(task_id) != expected_previous_writer ||
            MetadataInsertPreviousWriter(contract) != expected_previous_writer ||
            MetadataWriterPredecessorCount(task_id) != expected_wait_count ||
            LatestMetadataWriterPredecessorTask(task_id) != expected_latest) {
            return false;
        }
        predecessor_waits += expected_wait_count;
        ...
        expected_previous_writer = static_cast<int32_t>(task_id);
    }
    return writer_tasks == kDefaultBatches && writer_records == 3U * kDefaultBatches &&
           predecessor_waits == kDefaultBatches - 1U &&
...
int main(int argc, char **argv) {
    uint32_t rounds = 0U;
    if (!ParseRounds(argc, argv, &rounds) || !TestOrdinaryGlobalWriterContract() || !TestResetPreservesDispatch() ||
        !TestStaticBuilderPartition() || !TestHalfPacketAndUniqueDispatch() ||
        !TestTokenWindowWaitingBuilt() ||
    ...
    std::printf(
        "[PASS] GM CPU complete: builders=1..32, B1/B256, leaders/builder=%u, unique static build assignment, "
        "global sparse task-id writer chain, 8-shard heap, exact DAG/payload, %u-token tickets, "
        "fanin/completion/drain/tail, "
        "same-address reuse rounds=%u\n",
        kBuilderLeaderCount, kTokensPerOwner, rounds
    );
    return EXIT_SUCCESS;
}
