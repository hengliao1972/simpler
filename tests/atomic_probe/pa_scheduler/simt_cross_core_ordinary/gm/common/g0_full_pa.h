...
#ifndef PA_SCHEDULER_SIMT_CROSS_CORE_ORDINARY_G0_FULL_PA_H
#define PA_SCHEDULER_SIMT_CROSS_CORE_ORDINARY_G0_FULL_PA_H
...
namespace pa_scheduler::simt_cross_core_ordinary::g0 {
...
ExecutionWitnessState(
    uint64_t nonce, uint32_t task_id, TaskKind kind, uint32_t execute_owner, uint32_t execution_count,
...
// task 间距。ordinary 方案把所有真实 metadata writer 按 task-id 串成一条
// 全局链，因此 contract 同时携带 writer 数和 previous metadata writer。
// 每个 symbol 的精确 previous writer 仍随 history 发布，供依赖查询和 oracle
// 使用，不能用全局前驱替代真实 Tensor 版本关系。全局单写 commit 区间内的
// 派生 last_writer 可用 non-cacheable store 发布，不需要重复 CAS。
...
constexpr uint32_t kMetadataInsertPreviousWriterShift = kMetadataInsertWriterCountBits;
constexpr uint32_t kMetadataInsertPreviousWriterBits = 16U;
constexpr uint64_t kMetadataInsertPreviousWriterValueMask =
    (uint64_t{1} << kMetadataInsertPreviousWriterBits) - 1U;
constexpr uint64_t kMetadataInsertPreviousWriterMask =
    kMetadataInsertPreviousWriterValueMask << kMetadataInsertPreviousWriterShift;
constexpr uint64_t kMetadataInsertContractAllowedMask =
    kMetadataInsertContractPresent | kMetadataInsertWriterCountMask | kMetadataInsertPreviousWriterMask;
static_assert(kMaxTasks < kMetadataInsertPreviousWriterValueMask, "metadata predecessor encoding is too narrow");
...
SIMT_CROSS_CORE_G0_ABI_INLINE int32_t PreviousMetadataWriterTask(uint32_t task_id) {
    ...
    while (candidate != 0U) {
        --candidate;
        if (MetadataWriterIntentCount(candidate) != 0U) {
...
SIMT_CROSS_CORE_G0_ABI_INLINE uint32_t MetadataWriterPredecessorCount(uint32_t task_id) {
    return MetadataWriterIntentCount(task_id) != 0U && PreviousMetadataWriterTask(task_id) >= 0 ? 1U : 0U;
}
...
SIMT_CROSS_CORE_G0_ABI_INLINE uint32_t LatestMetadataWriterPredecessorTask(uint32_t task_id) {
    ...
    const int32_t previous = PreviousMetadataWriterTask(task_id);
    return previous < 0 ? UINT32_MAX : static_cast<uint32_t>(previous);
}
...
SIMT_CROSS_CORE_G0_ABI_INLINE int32_t MetadataInsertPreviousWriter(uint64_t contract) {
...
#endif  // PA_SCHEDULER_SIMT_CROSS_CORE_ORDINARY_G0_FULL_PA_H
