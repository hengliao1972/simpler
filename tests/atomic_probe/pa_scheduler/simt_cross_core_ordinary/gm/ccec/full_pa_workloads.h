...
#ifndef PA_SCHEDULER_SIMT_CROSS_CORE_ORDINARY_FULL_PA_WORKLOADS_H
#define PA_SCHEDULER_SIMT_CROSS_CORE_ORDINARY_FULL_PA_WORKLOADS_H

namespace pa_scheduler::simt_cross_core_ordinary::g0::device {
...
static __aicore__ __attribute__((noinline, used)) void RunG0CubeMatmul(
    ...
    // This is a duration probe, not an operator implementation to optimize.
    // Keep every repeat serialized so QK/PV continue to model the calibrated
    // task time.  Overlapping adjacent repeats changes the benchmark load even
    // when the nominal instruction count remains unchanged.
    for (uint32_t iteration = 0U; iteration < repeats; ++iteration) {
...
#endif  // PA_SCHEDULER_SIMT_CROSS_CORE_ORDINARY_FULL_PA_WORKLOADS_H
