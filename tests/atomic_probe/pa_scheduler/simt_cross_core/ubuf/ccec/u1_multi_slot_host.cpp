...
void InitializeGuard(U1Guard *guard, uint64_t nonce, uint32_t guard_id) {
    for (uint32_t word = 0U; word < kWordsPerLine; ++word) {
        guard->words[word] = ExpectedGuardWord(nonce, guard_id, word);
    }
}

bool ValidateGuard(const U1Guard &guard, uint64_t nonce, uint32_t guard_id, Failure *failure) {
    for (uint32_t word = 0U; word < kWordsPerLine; ++word) {
...
void InitializeState(U1ProbeState *state, uint64_t nonce) {
    ...
    for (uint32_t task_id = 0U; task_id < kTaskCount; ++task_id) {
        U1Task &task = state->tasks[task_id];
    ...
    for (uint32_t slot = 0U; slot < kUbufSlotCount; ++slot) {
        InitializeAtomicLine(&state->slot_states[slot], SlotFreeState(0U));
        InitializeAtomicLine(&state->slot_acquire_count[slot], 0U);
        InitializeAtomicLine(&state->slot_release_count[slot], 0U);
    }
    std::array<g0::AtomicLine *, 8U> atomics{
        &state->global_busy_depth,
        &state->global_max_busy_depth,
        &state->anchor_staged_count,
        &state->anchor_staged_mask,
    ...
    for (U1RoleReport &role : state->roles) {
        FillBytes(&role, kReportPoisonByte);
    }
    for (U1ThreadReport &report : state->builder_threads) {
        FillBytes(&report, kReportPoisonByte);
    }
}

bool ValidateControl(const U1Control &control, uint64_t nonce, Failure *failure) {
    const bool valid =
        ...
        control.ubuf_slot_count == kUbufSlotCount &&
        control.transport_kind == TransportKind::SimtUbufReadToGmWordStore &&
        control.ubuf_slot_stride_bytes == kUbufSlotStrideBytes && control.ubuf_region_bytes == kUbufRegionBytes &&
        control.ubuf_payload_offset_bytes == kUbufPayloadOffsetBytes;
    return valid ? true : Fail(failure, "control-mismatch", kInvalidTaskId, 0U, kProbeMagic, control.magic);
}

bool ValidateRole(
    const U1RoleReport &report, ProbeRole role, uint32_t owner, uint32_t subblock, uint32_t claims, uint32_t finishes,
...
bool ValidateRun(const U1ProbeState &state, uint64_t nonce, Failure *failure) {
    ...
    for (uint32_t slot = 0U; slot < kUbufSlotCount; ++slot) {
        const DecodedSlotState decoded = DecodeSlotState(state.slot_states[slot].value);
        if (!decoded.valid || decoded.busy || decoded.generation != kSlotReuseCount ||
            !ValidateAtomicLine(state.slot_states[slot], SlotFreeState(kSlotReuseCount), slot, failure) ||
            !ValidateAtomicLine(state.slot_acquire_count[slot], kSlotReuseCount, kUbufSlotCount + slot, failure) ||
            !ValidateAtomicLine(state.slot_release_count[slot], kSlotReuseCount, 2U * kUbufSlotCount + slot, failure)) {
            return false;
        }
    }
    const std::array<const g0::AtomicLine *, 8U> atomics{
        &state.global_busy_depth,
        &state.global_max_busy_depth,
        &state.anchor_staged_count,
        &state.anchor_staged_mask,
    ...
    const std::array<uint64_t, 8U> expected_atomic_values{
        0U, kUbufSlotCount, kAnchorTaskCount, kAnchorStagedMask, kTaskCount, kTaskCount, kTaskCount, 0U,
    };
    for (uint32_t index = 0U; index < atomics.size(); ++index) {
        if (!ValidateAtomicLine(*atomics[index], expected_atomic_values[index], 3U * kUbufSlotCount + index, failure)) {
            return false;
        }
    }

    // The transport implementation and optimized bitcode contain no MTE3;
    // keep an independent host-side expected value for the build gate/report.
    static_assert(kExpectedMte3Count == 0U, "U1 diagnostic transport must remain MTE3-free");
    std::array<std::array<bool, kSlotReuseCount>, kUbufSlotCount> generations{};
    for (uint32_t task_id = 0U; task_id < kTaskCount; ++task_id) {
        const U1Task &task = state.tasks[task_id];
        ...
        const U1BuildReport &build = task.build_report;
        const uint32_t expected_warp = task_id % kWarpCount;
        const uint32_t expected_thread = expected_warp * kWarpSize;
        const uint32_t expected_slot = SlotForTask(task_id);
        if (build.task_id != task_id || build.builder_thread != expected_thread ||
            build.builder_warp != expected_warp || build.builder_lane != 0U ||
            build.phase_bits != kExpectedBuildPhaseBits || build.payload_lines != payload_lines ||
            build.ubuf_words_written != payload_words || build.gm_words_stored != payload_words ||
            build.claim_count != 1U || build.publish_count != 1U || build.slot_id != expected_slot ||
            build.slot_generation >= kSlotReuseCount || (IsAnchorTask(task_id) && build.slot_generation != 0U) ||
        ...
        if (generations[build.slot_id][build.slot_generation]) {
            return Fail(failure, "duplicate-slot-generation", task_id, build.slot_generation, 0U, 1U);
        }
        generations[build.slot_id][build.slot_generation] = true;

        const U1ExecReport &exec = task.exec_report;
    ...
    for (uint32_t slot = 0U; slot < kUbufSlotCount; ++slot) {
        for (uint32_t generation = 0U; generation < kSlotReuseCount; ++generation) {
            if (!generations[slot][generation]) {
                return Fail(failure, "missing-slot-generation", kInvalidTaskId, generation, 1U, 0U);
            }
        }
    }

    for (uint32_t thread = 0U; thread < kThreadCount; ++thread) {
        const U1ThreadReport &report = state.builder_threads[thread];
        ...
        if (report.thread_id != thread || report.warp_id != warp || report.lane_id != 0U ||
            report.active_leader != 1U || report.first_task_id != warp || report.last_task_id != warp + kWarpCount ||
            report.task_count != kTasksPerWarp || report.status != kExpectedBuilderThreadStatus ||
            report.slot_attempt_count < kTasksPerWarp || report.build_count != kTasksPerWarp ||
            report.launch_nonce != nonce ||
            report.checksum !=
                ExpectedThreadChecksum(nonce, thread, warp, warp + kWarpCount, kExpectedBuilderThreadStatus) ||
            report.reserved != 0U) {
...
public:
    ...
    bool Initialize(int32_t device, const std::vector<char> &binary_data) {
        ...
        if (!CheckAcl(
                ...
                "aclrtBinaryLoadFromData(U1 mixed ELF)"
            ) ||
            !CheckAcl(aclrtBinaryGetFunctionByEntry(binary_, 0U, &function_), "aclrtBinaryGetFunctionByEntry(U1)") ||
            !CheckAcl(
                aclrtMalloc(&device_state_, sizeof(U1ProbeState), ACL_MEM_MALLOC_HUGE_FIRST),
                "aclrtMalloc(U1ProbeState)"
            )) {
            return false;
        }
        if ((reinterpret_cast<uintptr_t>(device_state_) & (kCacheLineBytes - 1U)) != 0U) {
            std::fprintf(stderr, "device U1ProbeState is not 64-byte aligned: %p\n", device_state_);
            return false;
        }
        return true;
    }

    bool Run(U1ProbeState *state) {
        if (!CheckAcl(
                aclrtMemcpy(device_state_, sizeof(*state), state, sizeof(*state), ACL_MEMCPY_HOST_TO_DEVICE),
                "aclrtMemcpy(H2D U1ProbeState)"
            )) {
            return false;
        }
        struct KernelArgs {
        ...
        static_assert(sizeof(KernelArgs) == sizeof(uint64_t), "unexpected U1 kernel argument ABI");
        return CheckAcl(
                   aclrtLaunchKernelWithHostArgs(function_, 1U, stream_, nullptr, &args, sizeof(args), nullptr, 0U),
                   "aclrtLaunchKernelWithHostArgs(U1 mixed 1:2)"
               ) &&
               CheckAcl(aclrtSynchronizeStream(stream_), "aclrtSynchronizeStream(U1)") &&
               CheckAcl(
                   aclrtMemcpy(state, sizeof(*state), device_state_, sizeof(*state), ACL_MEMCPY_DEVICE_TO_HOST),
                   "aclrtMemcpy(D2H U1ProbeState)"
               );
    }
...
int main(int argc, char **argv) {
    ...
    if (session.SocName().rfind("Ascend950", 0U) != 0U) {
        std::fprintf(stderr, "U1 requires A5/Ascend950, but ACL reports: %s\n", session.SocName().c_str());
        return EXIT_FAILURE;
    }

    std::printf(
        "[DEVICE] id=%d soc=%s topology=1*(1AIC+2AIV) state_bytes=%zu state=0x%llx "
        "builder_threads=%u warps=%u active_leaders=%u tasks=%u payload_lines=1/4/10/16 "
        "slots=%u slot_stride=%uB ubuf_region=%uB payload_offset=%uB alignment=%uB runs=%u\n",
        options.device, session.SocName().c_str(), sizeof(U1ProbeState),
        static_cast<unsigned long long>(session.DeviceStateAddress()), kThreadCount, kWarpCount, kWarpCount, kTaskCount,
        kUbufSlotCount, kUbufSlotStrideBytes, kUbufRegionBytes, kUbufPayloadOffsetBytes, kUbufAlignmentBytes,
        options.runs
    );
    std::printf(
        "[TRANSPORT] SIMT volatile UBUF store+load -> ordinary GM word store; MTE3 is intentionally absent "
        "and the independently checked expected MTE3 count is %llu\n",
        static_cast<unsigned long long>(kExpectedMte3Count)
    );

    auto state = std::make_unique<U1ProbeState>();
    uint32_t passes = 0U;
    for (uint32_t run = 0U; run < options.runs; ++run) {
        const uint64_t nonce = UINT64_C(0xA550553100000000) ^ (static_cast<uint64_t>(run) + 1U);
        ...
        if (!ValidateRun(*state, nonce, &failure)) {
            ...
            const DecodedU1Fatal decoded_fatal = DecodeU1Fatal(state->fatal.value);
            std::fprintf(
                stderr,
        ...
        ++passes;
        std::printf(
            "[PASS] run=%u nonce=0x%llx tasks=%u slots=%u final_generation=%u max_busy=%u "
            "acquire_release_per_slot=%u/%u built_done=%u/%u ubuf_guard_checks=%u mte3=0 "
            "same_device_address=yes\n",
            run + 1U, static_cast<unsigned long long>(nonce), kTaskCount, kUbufSlotCount, kSlotReuseCount,
            kUbufSlotCount, kSlotReuseCount, kSlotReuseCount, kTaskCount, kTaskCount, kTaskCount
        );
    }
    std::printf(
        "[SUMMARY] U1 passes=%u/%u same_address_reuse=%s builder_scalar_build_telemetry=0 "
