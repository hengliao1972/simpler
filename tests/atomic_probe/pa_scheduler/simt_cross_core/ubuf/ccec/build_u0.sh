...
KERNEL_SOURCE="$SCRIPT_DIR/u0_single_slot_kernel.cpp"
HOST_SOURCE="$SCRIPT_DIR/u0_single_slot_host.cpp"
COMMON_HEADER="$UBUF_ROOT/common/u0_single_slot.h"
AIC_OBJECT="$BUILD_DIR/simt_cross_core_u0_aic.o"
AIV_OBJECT="$BUILD_DIR/simt_cross_core_u0_aiv.o"
AIC_BITCODE="$BUILD_DIR/simt_cross_core_u0_aic.bc"
AIV_BITCODE="$BUILD_DIR/simt_cross_core_u0_aiv.bc"
AIC_BITCODE_DUMP="$BUILD_DIR/simt_cross_core_u0_aic.bc.dump"
AIV_BITCODE_DUMP="$BUILD_DIR/simt_cross_core_u0_aiv.bc.dump"
KERNEL_ELF="$BUILD_DIR/simt_cross_core_u0_kernel.o"
HOST_BINARY="$BUILD_DIR/simt_cross_core_u0_host"
BUILD_MANIFEST="$BUILD_DIR/u0_build_manifest.sha256"
AIC_ENTRY="simt_cross_core_u0_0_mix_aic"
AIV_ENTRY="simt_cross_core_u0_0_mix_aiv"
U0_BUILD_INPUTS=(
    run.sh
    common/full_pa_exec_protocol.h
    ubuf/common/u0_single_slot.h
    ubuf/ccec/u0_single_slot_kernel.cpp
    ubuf/ccec/u0_single_slot_host.cpp
    ubuf/ccec/build_u0.sh
)
...
echo "[CHECK] U0 pure-SIMT builder, one volatile UBUF slot and direct-GM diagnostic transport"
if rg -n '#include.*(cross_core|ops-nn|kernel_operator)' "$UBUF_ROOT" -g '*.h' -g '*.cpp'; then
    echo "U0 must not include cross_core, ops-nn or AscendC implementation headers." >&2
    exit 1
fi
if ! grep -Fq 'LAUNCH_BOUND(kThreadCount) void U0SimtBuildSingleSlot(' "$KERNEL_SOURCE" ||
   ...
   ! grep -Fq 'constexpr uint32_t kThreadCount = kWarpSize * kWarpCount;' "$COMMON_HEADER"; then
    echo "U0 must launch 2048 threads and activate only lane0 of each of 64 warps." >&2
    exit 1
fi
if ! grep -Fq '__ubuf__ volatile uint64_t *staging_region' "$KERNEL_SOURCE" ||
   ...
   ! grep -Fq 'TransportKind::SimtUbufReadToGmWordStore' "$HOST_SOURCE"; then
    echo "U0 must retain an observable volatile UBUF store/load followed by direct GM word stores." >&2
    exit 1
fi
if grep -Fq 'for (uint32_t word = 0U; word < kMaxPayloadWords; ++word)' "$KERNEL_SOURCE" ||
   ! grep -Fq 'kUbufRegionBytes == 4480U' "$COMMON_HEADER" ||
   ! grep -Fq 'state->control.ubuf_alignment_bytes == kUbufAlignmentBytes' "$KERNEL_SOURCE" ||
   ! grep -Fq 'U0FatalReason::InvalidTaskState' "$KERNEL_SOURCE" ||
   ! grep -Fq 'ubuf_guard_check_count' "$KERNEL_SOURCE" ||
   ! grep -Fq 'DecodeU0Fatal' "$HOST_SOURCE"; then
    echo "U0 must expose its guarded UBUF ABI, write only valid payload words and fail closed with one fatal ABI." >&2
    exit 1
fi
if rg -n 'copy_ubuf_to_gm|PIPE_MTE3|TSTORE|SET\.LOOP.*UBTOOUT|MOV\.UB\.TO\.OUT' \
        "$KERNEL_SOURCE" "$COMMON_HEADER"; then
    echo "U0 diagnostic transport must not contain an MTE3/UBTOOUT implementation." >&2
    exit 1
fi
if ! grep -Fq '&state.mte3_count' "$HOST_SOURCE" ||
   ! grep -Fq 'constexpr uint64_t kExpectedMte3Count = 0U;' "$HOST_SOURCE" ||
   [[ "$(grep -Fc 'kExpectedMte3Count' "$HOST_SOURCE")" -lt 2 ]]; then
    echo "U0 host must independently require the atomic-only mte3_count to stay zero." >&2
    exit 1
fi
if ! grep -Fq 'kBuilderOwner) << g0::kStateBuildOwnerShift' "$KERNEL_SOURCE"; then
    echo "all U0 task states must use physical AIV0 owner32; warp ids belong only in reports/tickets." >&2
    exit 1
fi
if ! grep -Fq '__aicore__ void RunExecutor(__gm__ U0ProbeState *state)' "$KERNEL_SOURCE" ||
...
if ! grep -Fq 'main_scalar_build_action_count == 0U' "$HOST_SOURCE" ||
   ! grep -Fq 'subblock, 0U, 0U, timeout_count' "$KERNEL_SOURCE"; then
    echo "U0 must prove the builder Main Scalar performs zero task-build actions." >&2
    exit 1
fi
...
source_line() {
    ...
    if [[ -z "$line" ]]; then
        echo "U0 source-order check cannot find: $needle" >&2
        exit 1
    fi
    printf '%s' "$line"
}
...
source_last_line() {
    local needle="$1"
    local line
    line="$(grep -nF "$needle" "$KERNEL_SOURCE" | tail -n 1 | cut -d: -f1)"
    if [[ -z "$line" ]]; then
        echo "U0 source-order check cannot find: $needle" >&2
        exit 1
    fi
...
busy_release_line="$(source_last_line 'asc_atomic_add(slot_busy, UINT64_MAX)')"
owner_release_line="$(source_last_line 'asc_atomic_cas(slot_owner, slot_value, kSlotFree)')"
...
if ! (( ubuf_store_line < ubuf_load_line && ubuf_load_line < gm_store_line && gm_store_line < fence_line &&
        fence_line < built_line && built_line < busy_release_line && busy_release_line < owner_release_line &&
        claim_line < dcci_line && dcci_line < payload_read_line && payload_read_line < done_line )); then
    echo "U0 source order must remain UBUF-store < UBUF-load < GM-store < fence < BUILT < release and Claim < DCCI < read < DONE." >&2
    exit 1
fi
if ! grep -Fq 'uint32_t runs = 100U;' "$HOST_SOURCE" ||
   ! grep -Fq 'inactive-lane-mutated' "$HOST_SOURCE" ||
   ! grep -Fq 'payload-tail' "$HOST_SOURCE" ||
   ! grep -Fq 'duplicate-slot-ticket' "$HOST_SOURCE"; then
    echo "U0 host must cover 100-round reuse, inactive-lane poison, payload tail and unique slot tickets." >&2
    exit 1
fi

echo "[BUILD] CCEC U0 AIC observer (dav-c310-cube)"
"$CCEC" "${COMMON_DEVICE_FLAGS[@]}" --cce-aicore-arch=dav-c310-cube \
    -o "$AIC_OBJECT" "$KERNEL_SOURCE"

echo "[BUILD] CCEC U0 AIV0 SIMT builder + AIV1 Scalar executor (dav-c310-vec)"
"$CCEC" "${COMMON_DEVICE_FLAGS[@]}" --cce-aicore-arch=dav-c310-vec \
    -o "$AIV_OBJECT" "$KERNEL_SOURCE"

echo "[BUILD] CCEC U0 optimized bitcode inventory"
"$CCEC" "${COMMON_DEVICE_FLAGS[@]}" --cce-aicore-arch=dav-c310-cube \
...
for symbol in "${required_aiv_symbols[@]}"; do
    if ! grep -Fq "$symbol" "$AIV_BITCODE_DUMP"; then
        echo "optimized U0 AIV bitcode is missing: $symbol" >&2
        exit 1
    fi
done
...
for entry in "$AIC_ENTRY" "$AIV_ENTRY"; do
    if ! awk -v name="$entry" \
        ...
        echo "final U0 ELF must define one non-empty GLOBAL entry: $entry" >&2
        exit 1
    fi
    if [[ "$SECTIONS" != *".ascend.meta.$entry"* ]]; then
        echo "final U0 ELF is missing metadata: .ascend.meta.$entry" >&2
        exit 1
    fi
done
...
if [[ "$global_functions" != "$expected_global_functions" ]]; then
    echo "final U0 ELF exports unexpected GLOBAL functions:" >&2
    printf '%s\n' "$global_functions" >&2
    exit 1
fi
if ! awk '$4 == "FUNC" && $5 == "LOCAL" && $3 + 0 > 0 && $NF ~ /U0SimtBuildSingleSlot.*_simt_entry$/ {count++}
         END {exit count != 1}' <<<"$SYMBOLS"; then
    echo "final U0 ELF must retain one non-empty local SIMT builder with an AS6 UBUF argument." >&2
    exit 1
fi
undefined_globals="$(awk '$5 == "GLOBAL" && $7 == "UND" {print $NF}' <<<"$SYMBOLS")"
if [[ -n "$undefined_globals" ]]; then
    echo "final U0 ELF contains undefined GLOBAL symbols:" >&2
    printf '%s\n' "$undefined_globals" >&2
    exit 1
fi
if [[ "$RELOCATIONS" != *"There are no relocations"* ]]; then
    echo "final U0 ELF unexpectedly retains relocations." >&2
    exit 1
fi
...
if [[ "$(grep -Fc 'KERNEL_TYPE: MIX_AIC_MAIN' <<<"$METADATA_OUTPUT")" -ne 2 ||
    ...
    echo "U0 mixed metadata is not an exact pair of MIX_AIC_MAIN [1:2] entries:" >&2
    printf '%s\n' "$METADATA_OUTPUT" >&2
    exit 1
fi
...
if [[ "$AIV_META_HEX" != *"0c000400 04000000"* || "$AIV_META_HEX" != *"07000400 00200000"* ]]; then
    echo "U0 AIV metadata must encode SIMD_SIMT_MIX_VF=4 and 8 KiB SIMT share memory." >&2
    printf '%s\n' "$AIV_META_HEX" >&2
    exit 1
fi
UBUF_REGION_BYTES=$((70 * 64))
SIMT_SHARE_BYTES=$((8 * 1024))
MAX_SHARE_BYTES=$((224 * 1024))
if (( UBUF_REGION_BYTES != 4480 || UBUF_REGION_BYTES > SIMT_SHARE_BYTES ||
      SIMT_SHARE_BYTES > MAX_SHARE_BYTES )); then
    echo "U0 guarded UBUF region must fit the compiler-emitted share memory and preserve the 32 KiB DCache floor." >&2
    exit 1
fi
echo "[CHECK] ELF has exact mixed entries; the guarded 4.375 KiB region fits TLV7=8 KiB share memory"

echo "[BUILD] GCC 15 U0 ACL host ($("$GXX15" -dumpfullversion))"
...
if [[ ! -s "$KERNEL_ELF" || ! -x "$HOST_BINARY" ]]; then
    echo "U0 build did not produce the required kernel and host artifacts." >&2
    exit 1
fi

manifest_tmp="$(mktemp "$BUILD_DIR/u0_build_manifest.XXXXXX")"
trap 'rm -f -- "$manifest_tmp"' EXIT
(
    cd "$SIMT_ROOT"
    sha256sum \
        "${U0_BUILD_INPUTS[@]}" \
        ubuf/build/ccec/simt_cross_core_u0_kernel.o \
        ubuf/build/ccec/simt_cross_core_u0_host
) > "$manifest_tmp"
mv -f -- "$manifest_tmp" "$BUILD_MANIFEST"
trap - EXIT

echo "[BUILD] U0 CCEC complete"
