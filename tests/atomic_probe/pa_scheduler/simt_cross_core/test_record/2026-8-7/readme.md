...
该值与 cross_core 正式 real-compute 默认值一致。当前源码没有 workload 命令行
...
./tests/atomic_probe/pa_scheduler/simt_cross_core/run.sh run-gm \
  --builders 8 --device 0 --batches 256 --runs 11

SIMT_CROSS_CORE_GM_BUILDER_WARPS=5 \
  ./tests/atomic_probe/pa_scheduler/simt_cross_core/run.sh build-gm-swimlane

./tests/atomic_probe/pa_scheduler/simt_cross_core/run.sh run-gm-swimlane \
