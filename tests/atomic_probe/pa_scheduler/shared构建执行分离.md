...
| 正式实现 | S0–S6.33 已形成中央 ticket + 严格 TensorMap 插入 + K2 异核 Execute；执行扫描得到的完整 control 快照直接参与 Claim CAS；execution drain 采用 16 条分组 arrival line 加 1 条独立 release line，由固定 root 完成终态校验；descriptor 引用、unique-ticket 单 CAS 和发布 Exchange 非等待候选均已撤销 |
| CPU 正确性用例 | S1–S4 K2、S5a 对侧角色 Build 与 S5b 全 96 Scalar Build 门槛已完成 |
| A5 跨核发布探针 | S2 已完成，100 轮共 3200 case 通过 |
| A5 PA 功能/性能 | 旧 `2.323 ms` 是 Submit-only 数字，已退出性能裁决；当前唯一口径为首个 Submit 起点到 FinalDrain 结束。S6.33 相对 S6.32 的冻结交错 A/B 中，中位由 `2.554 ms` 降至 `2.428 ms`，改善 `4.944%`；功能与终态全部 PASS |
...
| 当前调度缺口 | 双 token Claim-first 已解决旧的 FinalDrain backlog；S6.29 已消除 BUILT 扫描与 Claim 之间的重复返回型 load，S6.32 已消除 drain arrival/release 的同行干扰，S6.33 又把 drain arrival 的同地址竞争从 96 路降为每地址 6 路；当前主要缺口继续转向仍承担真实业务判断且不能复用既有快照的返回型 Atomic |
...
- 即使两个 control 都是 atomic-only，若一个被持续 poll、另一个仍有并发 RMW，也不应共享 cache line。execution drain 因此固定为 16 条分组 `arrival` 与 1 条 `release` 各自独占 64B，不能为节省状态字节再合并。
...
已到达核随后只轮询独立的 `exec_drain.release` 行；固定 root worker 轮询
16 个分组计数，确认每组均为 6 后校验所有 task cell 终态，再唯一发布
release。该分组只降低同地址 RMW 并发度，不改变 96 核全到达、全量终态
校验或 device 退出合同；分组计数越界、重复到达和重复 release 仍是错误。
...
23. FinalDrain 同时证明 builder 停止生产、所有 kernel cell 全部 DONE、每核两个 execution token 均为 `IDLE`、engine 无 in-flight；每个 worker 只向所属 arrival group 到达一次，固定 root 看到 16 组各 6 个到达后才允许发布独占 cache line 的 release；
