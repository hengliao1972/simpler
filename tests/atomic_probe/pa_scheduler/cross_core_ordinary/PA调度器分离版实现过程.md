...
本文按阶段记录 `cross_core_ordinary` standalone 的实际开发过程、验证证据和遗留问题。目标是先在独立代码中证明“构建与执行可跨 Scalar 核转移”，再决定是否迁移到 Simpler；本文不使用尚未运行的测试结果替代证据。

当前目录固定保留 Host 在 kernel launch 前预制 `SharedHostTaskPlan` 的实现，
作为四种预制调度方式的 Scalar ordinary 性能基线。无 Host 预制计划的动态
实现已隔离到相邻的 `cross_core_ordinary_dynamic/`，本目录不再跟随该路径演进。
