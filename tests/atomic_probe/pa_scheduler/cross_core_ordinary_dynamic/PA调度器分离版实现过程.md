...
本文按阶段记录 `cross_core_ordinary_dynamic` standalone 的实际开发过程、
验证证据和遗留问题。目标是先在独立代码中证明“构建与执行可跨 Scalar 核
转移”，再决定是否迁移到 Simpler；本文不使用尚未运行的测试结果替代证据。

本目录只承载无 Host 预制计划的动态路径。当前落地起点是 S7 全 Scalar 真实
orchestration replay；真正独立的运行时 `TaskPlan -> Build -> Execute` 三级
pipeline 仍按本文记录的合同继续开发。Host 预制 `SharedHostTaskPlan` 的性能基线
保留在相邻的 `cross_core_ordinary/`，两套实现不通过宏或运行参数互相切换。
...
`cross_core_ordinary/` 是从当前已经跑通的 shared-only same-core standalone 机械
复制出的独立源码闭包。它与 `same_core/` 对称，直接承载完整 PA；
只服务于 S0–S2 的小型协议探针则放在 `cross_core_ordinary/protocol_probe/`。
后续只在 `cross_core_ordinary/` 中替换非 Alloc 的执行包发布、领取、绑定和完成路径，
...
完整 `cross_core_ordinary/run.sh build cpu` 也已通过，包括 PA adapter、ordered
...
完整 `cross_core_ordinary/cpu/build.sh perf-clock` 回归通过。该阶段仍是 CPU 状态机证明，
...
`cross_core_ordinary/cpu/build.sh` 全量通过，覆盖 source atomic/DCCI、host plan、
