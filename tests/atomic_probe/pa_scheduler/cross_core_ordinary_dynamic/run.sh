...
# converter/analyzer 属于 pa_scheduler 公共工具；
# cross_core_ordinary_dynamic/ 只拥有动态 TaskPlan 实现。
...
本目录固定为 cross-core ordinary 动态 TaskPlan PA：
...
ccec_artifact_failure() {
    local variant="$1"
    local reason="$2"
    echo "Invalid CCEC cross-core ordinary dynamic artifact set ($variant): $reason" >&2
...
case "$ACTION" in
    ...
    swimlane)
        ...
        OUTPUT_ROOT="$PA_SCHEDULER_DIR/outputs/pa_scheduler_cross_core_ordinary_dynamic_swimlane_$(date -u +%Y%m%d_%H%M%S)_$$"
