...
extern "C" __global__ __aicore__ void KERNEL_ENTRY(atomic_exch_same_line)(__gm__ uint32_t *gx)
{
    ...
    for (uint32_t trial = 0; trial < TRIALS; trial++) {
        for (uint32_t round = 0; round < ROUNDS; round++) {
            (void)atomicExch(&gx[SAME_BASE + participant], TrialValue(participant, trial, round));
        }
        dsb(DSB_ALL);
        ccec_sync_all();
        if (participant == 0) {
        ...
        for (uint32_t round = 0; round < ROUNDS; round++) {
            (void)atomicExch(&gx[separate_slot], TrialValue(participant, trial, round));
        }
        dsb(DSB_ALL);
        ccec_sync_all();
        if (participant == 0) {
        ...
        for (uint32_t round = 0; round < ROUNDS; round++) {
            (void)atomicExch(&gx[ORDERED_BASE + participant], TrialValue(participant, trial, round));
            dsb(DSB_ALL);
        }
        ccec_sync_all();
        if (participant == 0) {
