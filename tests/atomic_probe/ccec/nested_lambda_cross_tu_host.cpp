...
void Validate(const std::vector<uint32_t> &storage, Variant variant, atomic_probe::Result &result)
{
    ...
    exact &= ReadField(storage, Field::DispatcherCalls) ==
        nested_lambda_cross_tu_probe::ExpectedDispatcherCalls(variant);
    ...
    exact &= args_bytes > 0 && args_bytes < 32U * 1024U && args_bytes % 64U == 0;
    exact &= ReadField(storage, Field::VariantEcho) == static_cast<uint32_t>(variant);

    char label[120];
    std::snprintf(
        label, sizeof(label), "CCEC AIC cross-TU ABI variant=%s",
        nested_lambda_cross_tu_probe::VariantName(variant));
...
int main(int argc, char *argv[])
{
    ...
    if (!ParseVariant(argv[2], &variant)) {
        std::fprintf(stderr, "Unknown cross-TU variant: %s\n", argv[2]);
        return EXIT_FAILURE;
    }
    ...
    std::printf("=== Pure CCEC AIC Caller-Capture Transport Probe ===\n");
    ...
    result.Expect(cleanup_ok, "CCEC cross-TU ACL cleanup");
