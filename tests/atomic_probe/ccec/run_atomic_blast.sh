#!/bin/bash
# Build & run the ccec atomic_blast probe on A5 onboard hardware.
#
# Compiles atomic_blast.cpp with ccec -x cce (AIC + AIV), links into
# an AICore binary, then compiles the host launcher with g++ and runs it.
#
# Tests atomicMax / atomicAdd (lowercase CCEC builtins) blast radius.
#
# Usage:
#   ./run_atomic_blast.sh          # build + run
#   ./run_atomic_blast.sh run      # run only (skip build)
#
...
KERNEL_SRC="$SCRIPT_DIR/atomic_blast.cpp"
KERNEL_OBJ="$BUILD_DIR/atomic_blast_kernel.o"
HOST_SRC="$SCRIPT_DIR/atomic_blast_host.cpp"
HOST_BIN="$BUILD_DIR/atomic_blast_host"
...
echo "=== Running ccec atomic_blast probe ==="
"$HOST_BIN" "$KERNEL_OBJ"
