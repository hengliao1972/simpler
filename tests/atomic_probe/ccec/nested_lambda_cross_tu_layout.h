...
CROSS_TU_DEVICE constexpr uint32_t FieldIndex(Field field)
{
...
{
    return 0x100000000ULL + static_cast<uint64_t>(round) * 0x1000ULL +
        static_cast<uint64_t>(tensor_index) * 0x100ULL + 0x55ULL;
}
...
CROSS_TU_DEVICE constexpr uint64_t ContextSalt(uint32_t round)
{
    return 0xBADC000000000000ULL + round;
}
...
{
    return TensorDigest(round, 0) * 3ULL + TensorDigest(round, 1) * 5ULL +
        TensorDigest(round, 2) * 7ULL + ContextSalt(round);
}
...
constexpr uint64_t KernelEntry(Variant variant)
{
