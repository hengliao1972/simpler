#!/bin/bash
# Build & run the ccec concurrent_stress probe on A5 onboard hardware.
#
# Compiles concurrent_stress.cpp with ccec -x cce (AIC + AIV), links into
# an AICore binary, then compiles the host launcher with g++ and runs it.
#
# Tests tight-loop concurrent access using st_dev/ld_dev/dcci intrinsics.
#
# Usage:
#   ./run_concurrent_stress.sh    # build + run
#   ./run_concurrent_stress.sh run  # run only (skip build)
#
...
KERNEL_SRC="$SCRIPT_DIR/concurrent_stress.cpp"
KERNEL_OBJ="$BUILD_DIR/concurrent_stress_kernel.o"
HOST_SRC="$SCRIPT_DIR/concurrent_stress_host.cpp"
HOST_BIN="$BUILD_DIR/concurrent_stress_host"
...
echo "=== Running ccec concurrent_stress probe ==="
"$HOST_BIN" "$KERNEL_OBJ"
