...
### 15.30 S6.8 负向取证：提前让出未省出 K2 次候选首读收益

S6.7 的备选核先读取 execution control，确认 `BUILT` 与 Build owner 后再
让出一次。本轮尝试用 task-id 的静态 K2 映射，在这次共享读取之前完成相同
的一次本地让出。这样每个 task 最多省掉一次次候选返回型 control load，
FinalDrain 仍绕过让出，且不改变 K2 候选、TensorMap 插入或 Build 资格。

CCEC 构建和三个 A5 B256 正确性样本通过，Submit 分别为：

```text
3.642054, 3.668757, 3.648742 ms
median = 3.648742 ms
```

该结果没有优于 S6.7 十轮中位 `3.637995 ms`。同时，提前让出使 9 个原本
...
### 15.32 S6.10 负向取证：旁路预过滤未转化为 Submit 收益

生产候选只改 local tournament，保持 96 核逻辑资格、G8 root CAS、严格
TensorMap 插入链和 K2 执行协议不变。每组一个随 task 轮换的首选候选直接
CAS，其余候选分别试验 0/50/100/500 NOP 后 `ld_dev`；读到 `-1` 仍回退
CAS，读到本 task id 才跳过。CPU 全套门槛、CCEC AIC/AIV 编译、B1/B256
完整业务终态均通过。

同一时段重新编译的干净 S6.7 基线五轮为：

```text
3.675387, 3.641973, 3.599999, 3.682199, 3.675272 ms
median = 3.675272 ms
```

四个候选五轮中位为：

| 非首选 NOP | B256 中位 | 相对同段基线 |
|---:|---:|---:|
| 0 | 3.687956 ms | +0.345% |
| 50 | 3.680760 ms | +0.149% |
| 100 | 3.671037 ms | -0.115% |
| 500 | 3.671612 ms | -0.100% |

最好名义差值只有 `0.115%`，各组又不是成对交错采样，不能解释成稳定收益；
它们也没有优于 S6.7 已有十轮中位 `3.637995 ms`。500 NOP 虽在独立
微基准把每组 local CAS 收敛到 1 次，但真实 Submit 同时承担旁路读、分支、
错峰指令和执行推进节奏变化，局部收益被抵消。

生产、host 口径和 CPU 测试改动均已完整撤回并重编 CCEC 基线确认。保留
...
重建 S6.41 的 ordered/latest-writer 候选，再次验证严格插入链可以从
正确性上传递 producer output 发布顺序。但在当前 S6.46 底座上，
12+12 交错 A5 B256 trace-free 的基线/候选中位为
`1.434364/1.436281 ms`，候选回退 `0.134%`；均值回退
`0.580%`，只有 6/12 对更快。因此继续保留
`SharedFaninOutputPublishedLoad`，候选实现完整撤回。详见实现过程 S6.47。

### 15.46 用完整周期复审 insert handoff 非返回候选

在 S6.46 底座上重建“保留条件 CAS，但当前 owner 不消费旧值”候选。
12+12 交错 A5 B256 trace-free 的基线/候选中位为
`1.436211/1.439456 ms`，候选回退 `0.226%`；均值回退
`0.411%`。候选还会把同 task 异常旧值从当前 owner 延迟到下一
owner/host 才报告，在无收益时不放宽该合同。实现已完整撤回，详见
实现过程 S6.48。

### 15.47 用完整周期复审 output published 非返回候选

在 S6.46 底座上重建“published Exchange 只发射、不消费旧值”候选。
12+12 交错 A5 B256 trace-free 的基线/候选中位为
`1.443516/1.447342 ms`，候选回退 `0.265%`；均值回退
`0.458%`，只有 4/12 对更快。候选还会删除生产者对非法重复发布的
当场检测，因此完整撤回，继续保留 Exchange 返回值检查。详见实现过程
S6.49。

### 15.48 用完整周期复审 unique-ticket 单 CAS Build

在 S6.46 底座上重建“payload 发布前不写 BUILDING，只做
`EMPTY -> BUILT` 唯一 CAS”候选。12+12 交错 A5 B256 trace-free
基线/候选中位为 `1.431355/1.440325 ms`，候选回退 `0.627%`；
...
### 15.57 将中央 Build 的正常态 fatal 观察周期从 8 改为 16

在保留启动后立即检查、等待慢路低频检查、FinalDrain 最终检查和 watchdog
的前提下，只把成功热路的 global-fatal 周期从每 8 个 Build 改为每 16 个。
远端错误的额外闭合上界由 7 个 task 增为 15 个，但中央 ticket 总量有限，
不会造成无限生产或跳过最终错误观察。

CPU 全量和远端错误收敛测试、CCEC 两类构建及 A5 B256 完整终态均通过。
冻结周期 8 产物交错 12 对后，基线/候选中位为
`1011.814/1005.961 us`，改善 `0.579%`；均值改善 `1.150%`，配对差中位
`-12.724 us`，候选 `9/12` 对更快。full-swimlane 的 direct fatal Load
从 217 次降到 146 次，DCCI 仍为 6528 次，Build/kernel 仍为 1280/1024。

同阶段还证伪了三项表面低风险候选：不消费 shared-output atomic 旧值未复现
历史收益；删除等价 token 满槽扫描的 12 对配对中位反而为 `+1.166 us`；
用本地占用计数替代 GM 扫描只在 `3/6` 对获胜。三项代码均完整撤回，不把
源码缩短或 ELF 变小写成性能收益。详细数据见实现过程 S6.74。

后续周期 32 候选虽通过 CPU/CCEC，却在 6 对交错中只有 `2/6` 对更快，
周期 16/32 中位为 `1007.689/1014.067 us`，配对差中位 `+4.633 us`；同时
错误传播上界会从 15 扩到 31 个 Build。该候选完整撤回，不再继续枚举更长
周期。

已验证 dispatch 快照候选同样撤回：它固定省去每 task 一次 `encoded_meta` GM
...
### 15.67 否决 payload route 重复解析消减

执行包发布入口原先为同一个不可变 task 解析两次 function/engine route。
候选不读取 PA task kind 分支、固定 DAG、batch、核数或输出形状，只尝试复用
一次 route 验证结果。首版因改变 spec 活跃区间使最终 `.text` 增加 `256 B`，
未上板即撤回；第二版保持原验证顺序，AIC/AIV 发布函数各缩短 `32 B`，但最终
kernel `.text` 仍为 `0x4a038`。

CPU/CCEC 与 A5 B256 全部正确性终态通过。冻结 12 对完整周期中，基线/候选
...
### 15.69 否决按 worker 静态均分 Execute 双项批次

候选按角色任务数、worker 数和 worker 序号为每核确定双项 ticket 配额，使
B256 Execute 返回型 Atomic 从 `606` 次降到恰好 `512` 次，并消除全部终端
越界领取。协议不读取 PA task kind、固定 DAG、batch/核数或 tensor 形状；
CPU 还用 `7` 批/`4` worker 的 `2/2/2/1` 非整除分配验证了通用闭合。
CPU、CCEC、A5 B1/B256 的严格 TensorMap 插入、1024 kernel exactly-once 和
全部终态均通过。

该方案同时静态锁定了每核工作份额，牺牲角色内动态 work stealing；对任务
ready 时机和执行代价更不均匀的其他算子，泛化风险高于当前 PA 样本。最终
perf-clock `.text` 增加 `768 B`。冻结 S6.93 做 12 对反转顺序 A/B：基线/候选
中位为 `842.812/841.682 us`，候选表面快 `0.134%`；但候选均值慢
