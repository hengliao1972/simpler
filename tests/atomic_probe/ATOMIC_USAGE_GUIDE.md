...
5. `st_dev`/`WriteGmByPassDCache` 绕过 scalar DCache，但不因此获得跨核 coherence 或原子性；在
   当前 multi-AIV repeated-store 场景中也不能依赖精确终值保证。测试尚未确定根因就是 store 重排。
6. CCEC 单 AIV 独自运行、同址重复 `st_dev` 的当前永久用例是 `0/2000`；AscendC 尚无同等永久
   单 AIV 用例。不能把多 AIV 分-line失败扩大成“单 AIV 独自写已经证明乱序”。准确表述是：每个
   地址虽然只有一个 AIV 写，但在另一个活跃 AIV 同一 trial 反复写另一条 line 的场景中仍观察到错误。
...
当前固化的 CCEC 单 AIV 用例独自运行，对同一 4B 地址连续执行 257 次 `st_dev`：

- 仅循环末 DSB：`0/2000` mismatch；
- 每次写后 DSB：`0/2000` mismatch。
...
所以不能说“单核反复 `st_dev` 已证明不保序”，也不能说“一个地址只有一个 writer 就安全”。准确
结论是：**每个 AIV 虽然只负责自己的地址和 cacheline，但在另一个活跃 AIV 同一 trial 反复写另一条
cacheline 的场景中，当前 repeated-store 压力仍能出现终值错误。** 测试没有证明两核指令的时间重叠
是必要或充分条件。allocation 内 line offset 与本轮错误频率显著相关，底层 bank/set/store queue 等
机制尚未由测试确定。
...
3. `st_dev` 写完、通过 atomic flag/phase 交权之前，至少执行一次 `DataSyncBarrier<MemDsbT::ALL>()`
   或 `dsb(DSB_ALL)`。
4. 逐写 DSB 在当前同-line对照中为 `0/4000`，但它只是已测控制路径，不是已经查到的通用 ISA
   正确性证明。关键业务仍需保留相同拓扑和压力的精确回归用例。
...
记录的多 AIV repeated `st_dev` 终值问题。若同一地址需要多次覆盖且最后值必须精确，可评估
...
| 单 AIV 独自 repeated `st_dev` | CCEC 当前 `0/2000`；AscendC 无同等永久用例 | 只记录当前通过，不扩大保证 |
...
`dcci_atomic_clobber`、`st_dev_separate_line_stress` 定向关闭：
...
- repeated `st_dev` 单 AIV永久路径：`ccec/st_dev_same_line.cpp`
