...
extern "C" __aicore__ void kernel_entry(__gm__ int64_t *args) {
    constexpr uint32_t input_count = 16;
    __gm__ Tensor *dump_tensor = reinterpret_cast<__gm__ Tensor *>(args[input_count]);
    __gm__ float *dump = reinterpret_cast<__gm__ float *>(dump_tensor->buffer.addr) + dump_tensor->start_offset;

    // 真实读取每个 producer 的独立 region，使 16 条 fanin 不能被编译器
    // 或数值 golden 偶然掩盖。
    ...
    for (uint32_t scalar = 0; scalar < MAX_SCALAR_ARGS; ++scalar) {
        scalar_sum += static_cast<uint64_t>(args[input_count + 1U + scalar]);
    }
    dump[input_count] = static_cast<float>(scalar_sum);

    for (uint32_t offset = 0; offset < (input_count + 1U) * sizeof(float); offset += 64U) {
        dcci(reinterpret_cast<__gm__ uint8_t *>(dump) + offset, SINGLE_CACHE_LINE, CACHELINE_OUT);
    }
    pipe_sync();
}
